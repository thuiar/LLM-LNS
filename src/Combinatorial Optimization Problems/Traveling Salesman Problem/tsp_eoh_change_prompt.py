...
class Paras():
    def __init__(self):
        ...
        self.problem = 'tsp_construct'     #选定解决的问题
...
class GetData():
    ...
    def __init__(self,n_instance,n_cities):
        self.n_instance = n_instance #实例的数量
        self.n_cities = n_cities     #每个实例中城市的数量

    def generate_instances(self):
        np.random.seed(2024)
        instance_data = []
        for _ in range(self.n_instance): #对每一个实例都随机生成一下
            #随机生成城市的坐标
...
class TSPGLS():
    ...
    def __init__(self) -> None:
        ...
        self.instance_path = './TrainingData' #,instances=None,instances_name=None,instances_scale=None
...
class InterfaceEC():
    def __init__(self, pop_size, m, api_endpoint, api_key, llm_model, debug_mode, interface_prob, select,n_p,timeout,use_numba,**kwargs):
        ...
        # 设置LLM 需要的一些信息
        self.pop_size = pop_size                    #定义种群大小
        self.interface_eval = interface_prob        #TSPGLS()类型，可以调用evaluate函数对算法代码进行评估
