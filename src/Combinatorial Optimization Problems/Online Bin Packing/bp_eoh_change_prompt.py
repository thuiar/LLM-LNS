...
class Paras:
    def __init__(self):
        ...
        # General settings  #
        #####################
        self.method = 'eoh'  # Select the method to use
        self.problem = 'bp_online'  # Select the problem to solve
        self.selection = None  # Select individual selection method (how to select individuals for evolution)
        self.management = None  # Select population management method
        ...
        # self.ec_operators = None # evolution operators: ['e1','e2','m1','m2'], default = ['e1','m1'] # Made redundant by EOH.run logic
        # self.ec_operator_weights = None  # weights for operators, default = [1,1,1,1] # Made redundant
        self.ec_m = 2  # number of parents for 'e1' and 'e2' operators (if used), default = 2
        ...
        self.exp_output_path = "./Bin_Packing/rebuttal1"  # default folder for outputs
        self.exp_n_proc = 1
    ...
    def set_parallel(self):
        #############################################################################
        # Set the number of processes to the maximum available on the machine     #
        #############################################################################
        import multiprocessing  # Import locally as it's only used here
    ...
    def set_ec(self):
        ####################################################################################################
        # Set population management strategy, parent selection strategy, and evolution strategy with weights #
        ####################################################################################################
        ...
        # Current EOH.run() logic does not use ec_operators directly from here for 'eoh' method.
        # It uses hardcoded "cross" and "variation" for algorithm generation guided by evolved prompts.
        # Thus, 'e1','e2','m1','m2' setup for 'eoh' is commented out as redundant for current EOH.
    ...
    def set_evaluation(self):
        ###########################################################################
        # Set relevant parameters for population evaluation (based on the problem) #
        ###########################################################################
        if self.problem == 'bp_online':
            ...
            self.eva_timeout = 3000
            self.eva_numba_decorator = False # Assuming False for TSP if not specified
...
####################################################################################
# All right! Basic settings are done, let's start with prompt settings             #
####################################################################################

def create_folders(results_path):
    ##################################################################################
    # Create results folder and create history, pops, pops_best subfolders within it #
    ##################################################################################
    folder_path = os.path.join(results_path, "results")

    # Check if the folder already exists
    if not os.path.exists(folder_path):
        # os.makedirs(folder_path) # Original code would error if parent doesn't exist
        # If you want to remove and recreate:
        # if os.path.exists(folder_path):
        #     shutil.rmtree(folder_path) # Requires import shutil
        os.makedirs(folder_path, exist_ok=True) # More robust
...
class GetPrompts:
    ...
    # Currently configured for an Online Bin Packing scoring function     #
    #######################################################################
    def __init__(self):
        # Prompt for task description
        self.prompt_task = (
            "I need help designing a novel score function that scores a set of bins to assign an item. "
        ...
        self.prompt_inout_inf = (
            "'item' and 'bins' are the size of current item and the rest capacities of feasible bins, "
            "which are larger than the item size. "
            "The output named 'scores' is the scores for the bins for assignment."
        )
        self.prompt_other_inf = (
            "Note that 'item' is of type int, while 'bins' and 'scores' are both Numpy arrays. "
            "The novel function should be sufficiently complex in order to achieve better performance. "
...
####################################################################################
# Time to start creating problem examples!                                         #
####################################################################################
...
class GetData:
    def __init__(self) -> None:
        ...
        # This function is defined but not called. If it's intended to be used,
        # it should be called, e.g., self.datasets = read_dataset_from_file(...)
        # For now, self.datasets['Weibull 5k'] remains empty.
        # The commented-out block below seems to be the intended data population.
        # I will keep this function definition as it might be used by uncommenting parts.
        def read_dataset_from_file(filename):
            with open(filename, 'rb') as file:
                dataset_loaded = pickle.load(file)

            transformed_dataset = {}
            for num_items, instances_list in dataset_loaded.items():
                dataset_key = f"Weibull {int(num_items / 1000)}k"
                transformed_dataset[dataset_key] = {}
                for instance_num, items_list in enumerate(instances_list, 1):
                    ...
                    transformed_dataset[dataset_key][instance_name] = instance_data
            return transformed_dataset

        # Example of how read_dataset_from_file might be used:
        # self.datasets = read_dataset_from_file('../weibull_instances/test_dataset_100k.pkl')

        # The following block is for populating/modifying 'Weibull 5k' specifically.
        # It assumes 'Weibull 5k' is already populated by read_dataset_from_file or similar.
        # If 'Weibull 5k' is meant to be hardcoded or loaded differently, this needs adjustment.
        # For this refactoring, I'm keeping it commented as in the original.
        # If test_dataset_100k.pkl actually contains a '5000' key that maps to the instances,
        # then the below processing would make sense after loading.
        # For now, `self.datasets['Weibull 5k']` is an empty dict.

        # for test_num in range(5): # This loop assumes 'Weibull 5k' has 'test_0' through 'test_4'
        #     test_key = f'test_{test_num}'
        #     # Ensure the key exists and data is loaded before processing
        #     if 'Weibull 5k' in self.datasets and test_key in self.datasets['Weibull 5k']:
        #         data = np.array(self.datasets['Weibull 5k'][test_key]['items'])
        #         # print(data) # Debug print
        #         # print(self.datasets['Weibull 5k'][test_key]['capacity']) # Debug print
        #         data = data / self.datasets['Weibull 5k'][test_key]['capacity']
        #         self.datasets['Weibull 5k'][test_key]['capacity'] = 1.0
        #         top_items = data[:1000] # Takes first 1000 items
        #         self.datasets['Weibull 5k'][test_key]['items'] = top_items
        #         self.datasets['Weibull 5k'][test_key]['num_items'] = len(top_items) # Update num_items
    ...
    def l1_bound_dataset(self, instances: dict) -> float:
        ...
        """
        if not instances: # Handle empty instances dict
            return 0.0
        l1_bounds = []
        for name in instances:
            instance = instances[name]
            l1_bounds.append(self.l1_bound(instance['items'], instance['capacity']))
        return np.mean(l1_bounds) if l1_bounds else 0.0

    def get_instances(self):
        opt_num_bins = {}
        for name, dataset in self.datasets.items():
            opt_num_bins[name] = self.l1_bound_dataset(dataset)
        print(opt_num_bins) # This will print {'Weibull 5k': 0.0} if data isn't loaded
        return self.datasets, opt_num_bins


class PROBLEMCONST:
    ####################################################################################################
    # Class for problem-specific constants and evaluation logic (e.g., Online Bin Packing)           #
    # Note: The original Chinese comment mentioned TSP, but the implementation is for Bin Packing.   #
    ####################################################################################################
    def __init__(self):
        getdata_obj = GetData() # Corrected variable name
        self.instances, self.lb = getdata_obj.get_instances()
        self.prompts = GetPrompts() # Prompts for guiding LLM algorithm generation

    def get_valid_bin_indices(self, item: float, bins: np.ndarray) -> np.ndarray:
        """Returns indices of bins in which item can fit."""
        return np.nonzero((bins - item) >= 0)[0]

    def online_binpack(self, items: tuple, bins: np.ndarray, alg_module): # Renamed alg to alg_module for clarity
        ...
        for item in items:
            ...
            if valid_bin_indices.size == 0:
                # This case should ideally not happen if enough bins are provided
                # or if problem constraints guarantee a fit.
                # Consider how to handle items that don't fit in any existing valid bin.
                # For now, this might lead to an error if alg_module.score expects non-empty bins.
                # Or, if an error occurs, it might be caught by the outer try-except in evaluate.
                # Assuming valid_bin_indices will not be empty due to problem setup (e.g. enough bins).
                # If it can be empty, alg_module.score needs to handle bins[valid_bin_indices] being empty.
                # Or, a new bin needs to be "opened" if that's part of the online problem spec.
                # For this refactor, I'll assume valid_bin_indices is non-empty.
                print(f"Warning: Item {item} could not fit in any available bin. This might indicate an issue.")
                continue # Skip this item or handle error appropriately

            # Score each bin based on heuristic.
            priorities = alg_module.score(item, bins[valid_bin_indices])
            # Add item to bin with highest priority.
            best_bin_index_in_valid = np.argmax(priorities)
            best_bin = valid_bin_indices[best_bin_index_in_valid]
    ...
    # @funsearch.run # FunSearch decorator, keep commented
    def evaluateGreedy(self, alg_module) -> float: # Renamed alg to alg_module
        """Evaluate heuristic function on a set of online binpacking instances."""
        overall_fitness_contributions = []

        for dataset_name, dataset_instances in self.instances.items():
            if not dataset_instances: # Skip if a dataset has no instances
                print(f"Warning: Dataset '{dataset_name}' is empty. Skipping evaluation for this dataset.")
                continue

            num_bins_list = []
            for _, instance in dataset_instances.items():
                capacity = instance['capacity']
                items = np.array(instance['items'])

                # Create num_items bins so there will always be space for all items,
                # regardless of packing order. Array has shape (num_items,).
                # Ensure num_items is at least 1 to avoid issues with empty arrays if items is empty.
                num_initial_bins = max(instance['num_items'], 1)
                bins = np.full(num_initial_bins, capacity, dtype=float)

                # Pack items into bins and return remaining capacity in bins_packed
                _, bins_packed = self.online_binpack(items, bins, alg_module)

                # If remaining capacity in a bin is equal to initial capacity, then it is unused.
                # Count number of used bins.
                used_bins_count = (bins_packed != capacity).sum()
                num_bins_list.append(-used_bins_count) # Negative because we want to minimize bins

            if not num_bins_list: # If no instances were processed in this dataset
                 # This might happen if dataset_instances was not empty but all its instances were somehow invalid/empty
                print(f"Warning: No valid instances processed for dataset '{dataset_name}'.")
                dataset_fitness = 0.0 # Or handle as an error, or skip contribution
            else:
                avg_neg_num_bins = np.mean(np.array(num_bins_list))
                # Fitness relative to lower bound (L1). We want to maximize this (minimize (avg_num_bins - lb)).
                # Since num_bins_list contains negative numbers, avg_neg_num_bins is negative.
                # avg_num_bins = -avg_neg_num_bins.
                # fitness = (avg_num_bins - self.lb[dataset_name]) / self.lb[dataset_name]
                # To maximize fitness (which is typically a positive value to be maximized by EC):
                # A common way for minimization problem is fitness = -objective_value
                # Here, objective is num_bins. So, fitness = -num_bins.
                # The original code's fitness calculation:
                # fitness = (avg_num_bins - self.lb[name]) / self.lb[name]
                # If avg_num_bins is close to lb, fitness is small. If avg_num_bins is much larger, fitness is larger.
                # This seems like a "percentage above lower bound", which should be minimized.
                # Let's assume the original fitness calculation is intended for a maximization context (e.g. AEL maximizes fitness).
                # If the goal is to *minimize* this value, then this fitness should be negated or used in a min-heap.
                # The EOH framework seems to sort by 'objective' and pick the smallest for minimization.
                # So, 'objective' should be the value to minimize.
                # Objective = (avg_num_bins - self.lb[dataset_name]) / self.lb[dataset_name]
                # avg_num_bins = -avg_neg_num_bins
                current_lb = self.lb.get(dataset_name, np.sum(items) / capacity if items.size > 0 else 1.0) # Fallback LB
                if current_lb == 0: current_lb = 1.0 # Avoid division by zero

                objective_value = (-avg_neg_num_bins - current_lb) / current_lb
                overall_fitness_contributions.append(objective_value)
        
        # Average fitness across all datasets
        if not overall_fitness_contributions:
            return 0.0 # Or some indicator of error/no evaluation
        return np.mean(overall_fitness_contributions)
    ...
    def evaluate(self, code_string):
        try:
            ...
            with warnings.catch_warnings():
                ...
                # Add the module to sys.modules so it can be imported (though not strictly necessary here)
                # sys.modules[heuristic_module.__name__] = heuristic_module # Not needed if not importing by name

                fitness = self.evaluateGreedy(heuristic_module)
                return fitness
        except Exception as e:
            # print(f"Error during evaluation: {str(e)} with code:\n{code_string}") # More informative debug
            return None # Return None or a very bad fitness value


class Probs:
    ##################################################################################
    # Reads problem instances or uses PROBLEMCONST to create problem instances       #
    ##################################################################################
    def __init__(self, paras):
        if not isinstance(paras.problem, str):
            # Read existing problem instances (if paras.problem is a pre-loaded object)
            self.prob = paras.problem
            print("- Prob local loaded.")
        elif paras.problem in ["tsp_construct", "bp_online"]: # Added bp_online
            # Create new problem instances
            self.prob = PROBLEMCONST()
            print(f"- Prob {paras.problem} loaded via PROBLEMCONST.")
        else:
            print(f"Problem {paras.problem} not found!")
            sys.exit(1) # Exit if problem not found
...
####################################################################################
# Finally, we've reached the section for interacting with the large model!         #
####################################################################################

class InterfaceAPI:
    ##############################################################
    # Call API to communicate with the large model               #
    ##############################################################
    def __init__(self, api_endpoint, api_key, model_LLM, debug_mode):
        ...
        self.n_trial = 5  # Maximum 5 attempts when trying to get a response

    def get_response(self, prompt_content):
        # Create a JSON formatted string payload_explanation for the request.
        payload_explanation = json.dumps(
            {
                "model": self.model_LLM,  # Specify the model to use.
                "messages": [  # Message content, representing user input.
        ...
        headers = {
            ...
            "User-Agent": "Apifox/1.0.0 (https://apifox.com)",  # Identifies client information.
            "Content-Type": "application/json",  # Specifies the request content type as JSON.
            "x-api2d-no-cache": "1",  # Custom header for controlling cache behavior.
        }

        response_content = None  # Initialize variable to store API response content
        
        for attempt in range(self.n_trial):
            try:
                ...
                # Extract the actual model reply content
                response_content = json_data["choices"][0]["message"]["content"]
                conn.close()
                break  # Success, exit loop
            except Exception as e:
                if self.debug_mode:
                    print(f"Error in API (attempt {attempt + 1}/{self.n_trial}): {e}. Retrying...")
                if attempt == self.n_trial - 1: # Last attempt
                    print(f"API request failed after {self.n_trial} attempts.")
                    return None # Return None after all trials fail
                time.sleep(1 + attempt) # Exponential backoff might be better
...
class InterfaceLLM:
    #######################################################################
    # Wrapper to call InterfaceAPI class for LLM communication            #
    #######################################################################
    def __init__(self, api_endpoint, api_key, model_LLM, debug_mode):
        self.api_endpoint = api_endpoint  # API endpoint URL
        ...
        if not api_key or not api_endpoint or api_key == 'your_api_key' or api_endpoint == 'your_llm_endpoint':
            print(">> Critical Error: LLM API settings are default/missing. Please set api_endpoint and api_key!")
            sys.exit(1)
        
        print('Remote LLM API is used.')
        # Create an instance of InterfaceAPI for handling API requests.
        self.interface_llm_api = InterfaceAPI(
            self.api_endpoint,
            self.api_key,
            self.model_LLM,
            self.debug_mode,
        ...
        # Test API connection
        res = self.interface_llm_api.get_response("1+1=?")
        if res is None:
            print(">> Error: LLM API connection failed. Check endpoint, key, model, or network.")
            sys.exit(1)
        else:
    ...
    def get_response(self, prompt_content):
        ##############################################################################
        # Get LLM's response to given content.                                       #
        # Accepts prompt_content (user-provided prompt).                             #
        ##############################################################################
        # Call InterfaceAPI's get_response method.
        response = self.interface_llm_api.get_response(prompt_content)
        return response


####################################################################################
# Outer layer for Prompt Evolution!                                                #
####################################################################################
class Evolution_Prompt:
    """
    Handles the evolution of prompts themselves, asking an LLM to generate new prompts
    based on existing ones.
    """
    def __init__(self, api_endpoint, api_key, model_LLM, debug_mode, problem_type):
        # problem_type: "minimization" or "maximization"
        self.prompt_task_meta = ( # Meta-prompt for evolving prompts
            ...
            " We have already developed a set of initial prompts (that are used to ask an LLM to generate algorithms) and observed the corresponding objective values of the algorithms generated by those prompts."
            " To improve the effectiveness of these algorithms, we need your assistance in carefully analyzing the existing prompts and their resulting algorithm performances."
            " Based on this analysis, we ask you to generate NEW PROMPTS (not algorithms) that will help us achieve better outcomes in solving the problem."
            " The new prompt you generate should be a concise instruction that can be given to another LLM to design an algorithm."
        )

        # Set large model parameters for evolving prompts
        self.api_endpoint = api_endpoint
        self.api_key = api_key
        self.model_LLM = model_LLM
        self.debug_mode = debug_mode

        # Use InterfaceLLM for communication
        self.interface_llm_prompter = InterfaceLLM(self.api_endpoint, self.api_key, self.model_LLM, self.debug_mode)

    def get_prompt_cross(self, parent_prompts_info):
        ##################################################
        # Generate LLM query for prompt crossover        #
        ##################################################
        prompt_indiv_desc = ""
        for i, p_info in enumerate(parent_prompts_info):
            prompt_indiv_desc += f"No.{i+1} prompt's task assigned to LLM: \"{p_info['prompt']}\"\n"
            prompt_indiv_desc += f"Objective function value of algorithm generated by this prompt: {p_info['objective']}\n"

        # Construct the full query to the LLM
        prompt_content = (
            f"{self.prompt_task_meta}\n"
            f"I have {len(parent_prompts_info)} existing prompts with their resulting algorithm objective function values as follows:\n"
            f"{prompt_indiv_desc}"
            "Please help me create ONE new prompt that has a totally different form from the given ones but can be motivated by them. "
            "The new prompt should be a single, concise instruction. "
            "Please provide ONLY the new prompt string itself, without any other explanation or formatting (e.g., no quotes around it unless they are part of the prompt itself).\n"
            "New Prompt:" # Added to guide LLM for direct output
        )
        return prompt_content

    def get_prompt_variation(self, parent_prompt_info):
        ######################################################
        # Generate LLM query for prompt variation/mutation   #
        ######################################################
        prompt_content = (
            f"{self.prompt_task_meta}\n"
            "I have one prompt with its resulting algorithm objective function value as follows:\n"
            f"Prompt description: \"{parent_prompt_info[0]['prompt']}\"\n"
            f"Objective function value of algorithm generated by this prompt: {parent_prompt_info[0]['objective']}\n"
            "Please assist me in creating ONE new prompt that has a different form but can be a modified version of the provided prompt. "
            "The new prompt should be a single, concise instruction. "
            "Please provide ONLY the new prompt string itself, without any other explanation or formatting.\n"
            "New Prompt:" # Added to guide LLM for direct output
        )
        return prompt_content

    def initialize(self, prompt_type):
        """Returns a list of initial seed prompts for guiding algorithm generation."""
        if prompt_type == 'cross':
            # Initial prompts that encourage diverse algorithm generation (for crossover-style algorithm generation)
            prompt_content_list = [
                'Please help me create a new algorithm that has a totally different form from the given ones.',
                'Please help me create a new algorithm that has a totally different form from the given ones but can be motivated from them.'
            ]
        else: # variation
            # Initial prompts that encourage modification/refinement (for variation-style algorithm generation)
            prompt_content_list = [
        ...
        return prompt_content_list

    def cross(self, parents_info):
        ###################################################################################################
        # Get a new prompt from LLM, using crossover-style inspiration from multiple parent prompts.      #
        ###################################################################################################
        # Get the query content to ask LLM for creating a new prompt
        prompt_query = self.get_prompt_cross(parents_info)
        
        if self.debug_mode:
            print("\n>>> Debug: Query to LLM for generating new prompt (cross-over style):\n", prompt_query)
            # print(">>> Press 'Enter' to continue") # Removed for non-interactive flow
            # input()

        new_prompt_str = self.interface_llm_prompter.get_response(prompt_query)
        
        if self.debug_mode and new_prompt_str:
            print("\n>>> Debug: New prompt generated by LLM (cross-over):\n", new_prompt_str)

        return new_prompt_str

    def variation(self, parent_info):
        ###################################################################################################
        # Get a new prompt from LLM, by asking it to vary/mutate a single parent prompt.                  #
        ###################################################################################################
        prompt_query = self.get_prompt_variation(parent_info)

        if self.debug_mode:
            print("\n>>> Debug: Query to LLM for generating new prompt (variation style):\n", prompt_query)
            # print(">>> Press 'Enter' to continue") # Removed for non-interactive flow
            # input()
        ...
        if self.debug_mode and new_prompt_str:
            print("\n>>> Debug: New prompt generated by LLM (variation):\n", new_prompt_str)

        return new_prompt_str


####################################################################################
# Interface for Evolving Prompts using EC-like mechanisms                          #
####################################################################################
class InterfaceEC_Prompt:
    def __init__(self, pop_size_prompts, m_parents_prompts, api_endpoint, api_key, llm_model, 
                 debug_mode, select_method, num_parallel_jobs, timeout, problem_type):

        # pop_size_prompts: how many new prompts to generate in one evolutionary step (e.g., via parallel calls)
        self.pop_size_prompts = pop_size_prompts
        self.m_parents_prompts = m_parents_prompts # Number of parent prompts for crossover

        self.evol_prompter = Evolution_Prompt(api_endpoint, api_key, llm_model, debug_mode, problem_type)
        self.debug = debug_mode

        if not self.debug:
            warnings.filterwarnings("ignore", category=UserWarning) # More specific warning suppression
            warnings.filterwarnings("ignore", category=DeprecationWarning)

        self.select_method = select_method # Parent selection method for prompts
        self.num_parallel_jobs = num_parallel_jobs
        self.timeout = timeout

    def add_to_prompt_population(self, prompt_population, offspring_prompt_dict):
        # Add a new offspring prompt (as a dict) to the existing population of prompts
        # Condition: no duplicate 'prompt' string.
        if offspring_prompt_dict is None or offspring_prompt_dict['prompt'] is None:
            if self.debug: print("Attempted to add an invalid/None prompt. Skipping.")
            return False

        for ind_prompt in prompt_population:
            if ind_prompt['prompt'] == offspring_prompt_dict['prompt']:
                if self.debug:
                    print("Duplicated prompt string, not adding to prompt population.")
                return False
        prompt_population.append(offspring_prompt_dict)
        return True

    def extract_clean_prompt(self, raw_llm_response_text):
        if raw_llm_response_text is None:
            return None
        # Try to extract from quotes first
        match = re.search(r'"(.*?)"', raw_llm_response_text, re.DOTALL)
        if match:
        ...
        prefixes_to_remove = ["New Prompt:", "Prompt:"]
        for prefix in prefixes_to_remove:
            if text.startswith(prefix):
                text = text[len(prefix):].strip()
        return text

    def _generate_single_offspring_prompt(self, prompt_pop, operator):
        # Initialize offspring prompt dictionary
        offspring_prompt = {
            'prompt': None,
            'objective': 1e9,  # Initial dummy objective for the prompt (performance of algorithm it generates)
            'number': [] # To store performance of multiple algorithms generated by this prompt
        }
        parent_prompts_info = None # For debug or logging

        if operator == "cross":
            parent_prompts_info = self.select_method.parent_selection(prompt_pop, self.m_parents_prompts)
            if not parent_prompts_info or len(parent_prompts_info) < self.m_parents_prompts:
                 if self.debug: print(f"Not enough parents for prompt crossover ({len(parent_prompts_info)}/{self.m_parents_prompts}). Returning None.")
                 return None, None
            new_prompt_text = self.evol_prompter.cross(parent_prompts_info)
            offspring_prompt["prompt"] = self.extract_clean_prompt(new_prompt_text)
        elif operator == "variation":
            parent_prompts_info = self.select_method.parent_selection(prompt_pop, 1)
            if not parent_prompts_info:
                if self.debug: print("No parent selected for prompt variation. Returning None.")
                return None, None
            new_prompt_text = self.evol_prompter.variation(parent_prompts_info)
            offspring_prompt["prompt"] = self.extract_clean_prompt(new_prompt_text)
        else:
            print(f"Prompt evolution operator [{operator}] has not been implemented! Returning None.")
            return None, None # Or raise error

        if offspring_prompt["prompt"] is None or not offspring_prompt["prompt"].strip():
            if self.debug: print(f"LLM failed to generate a valid prompt string for operator {operator}.")
            return parent_prompts_info, None # Return None for offspring if LLM fails

        return parent_prompts_info, offspring_prompt

    def _generate_initial_prompts_set(self, initial_prompt_type):
        """Generates a set of initial prompts based on predefined templates."""
        initial_prompts_text_list = self.evol_prompter.initialize(initial_prompt_type)
        offspring_prompts_set = []
        for prompt_text in initial_prompts_text_list:
            offspring_prompts_set.append({
                'prompt': prompt_text,
                'objective': 1e9, # Initial dummy objective
                'number': []
            })
        return None, None, offspring_prompts_set # Parents=None, SingleOffspring=None, OffspringSet=Filled

    def get_offspring_prompts(self, prompt_pop, operator):
        # This function handles the generation of one or more offspring prompts.
        # It adapts the structure from the original get_offspring to fit prompt evolution.
        parents_info = None
        single_offspring_prompt = None
        offspring_prompts_set = [] # For initial generation

        try:
            if operator == "initial_cross" or operator == "initial_variation":
                # Special handling for initial prompt generation
                initial_type = "cross" if operator == "initial_cross" else "variation"
                parents_info, single_offspring_prompt, offspring_prompts_set = self._generate_initial_prompts_set(initial_type)
            elif operator == "cross" or operator == "variation":
                parents_info, single_offspring_prompt = self._generate_single_offspring_prompt(prompt_pop, operator)
            else:
                 print(f"Unknown operator for prompt generation: {operator}")

        except Exception as e:
            if self.debug: print(f"Exception in get_offspring_prompts ({operator}): {e}")
            # Ensure a consistent return structure in case of error
            single_offspring_prompt = None # Reset to ensure it's None on error
            offspring_prompts_set = [] # Ensure it's empty on error if it was for initial set

        # The function now needs to return a structure that get_evolved_prompts expects.
        # Original: return p, offspring, off_set
        # p: parents_info
        # offspring: single_offspring_prompt (if not initial)
        # off_set: offspring_prompts_set (if initial)
        return parents_info, single_offspring_prompt, offspring_prompts_set


    def get_evolved_prompts(self, prompt_pop, operator):
        # results: Store generated offspring prompts
        results_tuples = []
        try:
            num_to_generate = 1
            if operator == 'initial_cross' or operator == 'initial_variation':
                 # For initial generation, it's one call that returns a set
                 num_to_generate = 1
            elif operator == 'cross' or operator == 'variation':
                 # For evolutionary steps, generate pop_size_prompts new prompts
                 num_to_generate = self.pop_size_prompts
            
            # Parallel generation of new prompts
            # Each call to get_offspring_prompts returns (parents_info, single_offspring, offspring_set)
            results_tuples = Parallel(n_jobs=self.num_parallel_jobs, timeout=self.timeout + 15)(
                delayed(self.get_offspring_prompts)(prompt_pop, operator) for _ in range(num_to_generate)
            )
        except Exception as e: # Catch timeout from Parallel itself
            if self.debug:
                print(f"Error in Parallel prompt generation ({operator}): {e}")
            print("Parallel prompt generation may have timed out or failed.")
            # results_tuples will remain empty or partially filled

        time.sleep(1) # Small delay after parallel processing, if needed

        all_parent_prompts_info = []
        all_new_offspring_prompts = []

        for p_info, single_off, off_set in results_tuples:
            if p_info: # Can be a list of parents or a single parent
                all_parent_prompts_info.append(p_info)
            
            if operator == 'initial_cross' or operator == 'initial_variation':
                # off_set contains the list of initial prompts
                if off_set: all_new_offspring_prompts.extend(off_set)
            else: # 'cross' or 'variation'
                # single_off is the newly generated prompt dict
                if single_off and single_off.get('prompt'): # Ensure valid prompt
                    all_new_offspring_prompts.append(single_off)
            
            if self.debug:
                if single_off: print(f">>> Debug: Generated single offspring prompt: \n {single_off}")
                if off_set: print(f">>> Debug: Generated initial prompt set: \n {off_set}")
                
        return all_parent_prompts_info, all_new_offspring_prompts


    def initial_prompt_population_generation(self, initial_type_operator):
        # initial_type_operator: "initial_cross" or "initial_variation"
        # This will generate ONE set of initial prompts based on the type.
        _, population_of_prompts = self.get_evolved_prompts([], initial_type_operator)
        
        # Filter out any None prompts if generation failed for some
        valid_population = [p for p in population_of_prompts if p and p.get('prompt')]
        
        if self.debug:
            print(f"Initial prompt population ({initial_type_operator}) generated {len(valid_population)} prompts.")
        return valid_population


####################################################################################
# Inner layer! For Algorithm Evolution using LLM, guided by prompts.               #
####################################################################################
class Evolution:
    """
    Handles the generation of algorithms (code and description) using an LLM,
    based on a given prompt and potentially parent algorithms.
    """
    def __init__(self, api_endpoint, api_key, model_LLM, debug_mode, problem_prompts):
        # problem_prompts: An instance of GetPrompts (or similar) defining the problem context
        self.prompt_task = problem_prompts.get_task()
        self.prompt_func_name = problem_prompts.get_func_name()
        self.prompt_func_inputs = problem_prompts.get_func_inputs()
        self.prompt_func_outputs = problem_prompts.get_func_outputs()
        self.prompt_inout_inf = problem_prompts.get_inout_inf()
        self.prompt_other_inf = problem_prompts.get_other_inf()

        # Format input/output lists for inclusion in LLM prompts
        self.joined_inputs = ", ".join(f"'{s}'" for s in self.prompt_func_inputs) if self.prompt_func_inputs else ""
        self.joined_outputs = ", ".join(f"'{s}'" for s in self.prompt_func_outputs) if self.prompt_func_outputs else ""
        
        # LLM interface for algorithm generation
        self.api_endpoint = api_endpoint
        self.api_key = api_key
        self.model_LLM = model_LLM
        self.debug_mode = debug_mode
        self.interface_llm_algo_gen = InterfaceLLM(self.api_endpoint, self.api_key, self.model_LLM, self.debug_mode)

    def get_prompt_initial_algorithm(self):
        ##################################################
        # Generate LLM query for initial algorithm       #
        ##################################################
        # Describes the task and requests LLM to:
        # 1. Describe new algorithm in one sentence (in braces).
        # 2. Implement in Python as self.prompt_func_name.
        # 3. Specify inputs/outputs.
        # 4. Describe I/O properties.
        # 5. Other properties.
    ...
    def get_prompt_cross_algorithm(self, parent_algorithms_info, guiding_prompt_text):
        ###################################################################
        # Generate LLM query for new algorithm via crossover inspiration  #
        ###################################################################
        prompt_indiv_desc = ""
        for i, alg_info in enumerate(parent_algorithms_info):
            prompt_indiv_desc += (
                f"No.{i+1} algorithm's thought: {alg_info['algorithm']}\n"
                f"Objective function value: {alg_info['objective']}\n"
                f"Corresponding code:\n{alg_info['code']}\n"
            )
        
        prompt_content = (
            f"{self.prompt_task}\n"
            f"I have {len(parent_algorithms_info)} existing algorithms with their thoughts, objective values, and codes as follows:\n"
            f"{prompt_indiv_desc}"
            f"Based on these, and inspired by the following idea: \"{guiding_prompt_text}\"\n"
            "First, describe your new algorithm and main steps in one sentence. The description must be inside a brace {}. "
            f"Next, implement it in Python as a function named {self.prompt_func_name}. "
            f"This function should accept {len(self.prompt_func_inputs)} input(s): {self.joined_inputs}. "
            f"The function should return {len(self.prompt_func_outputs)} output(s): {self.joined_outputs}. "
    ...
    def get_prompt_variation_algorithm(self, parent_algorithm_info, guiding_prompt_text):
        ######################################################################
        # Generate LLM query for new algorithm via variation/mutation      #
        ######################################################################
        prompt_content = (
            f"{self.prompt_task}\n"
            "I have one algorithm with its code as follows:\n"
            f"Algorithm description: {parent_algorithm_info['algorithm']}\n"
            f"Code:\n{parent_algorithm_info['code']}\n"
            f"Objective value: {parent_algorithm_info['objective']}\n"
            f"Please create a new algorithm, potentially a modified version, based on the following suggestion: \"{guiding_prompt_text}\"\n"
            "First, describe your new algorithm and main steps in one sentence. The description must be inside a brace {}. "
            f"Next, implement it in Python as a function named {self.prompt_func_name}. "
            f"This function should accept {len(self.prompt_func_inputs)} input(s): {self.joined_inputs}. "
            f"The function should return {len(self.prompt_func_outputs)} output(s): {self.joined_outputs}. "
    ...
    def _extract_algorithm_and_code(self, llm_response):
        # Get response from LLM interface for the given prompt_content.
        if llm_response is None:
            return None, None

        # Try to extract algorithm description from braces {}
        algorithm_desc_match = re.search(r"\{(.*?)\}", llm_response, re.DOTALL)
        algorithm_desc = None
        if algorithm_desc_match:
            algorithm_desc = algorithm_desc_match.group(1).strip()
        else:
            # Fallback extraction for algorithm description
            if 'python' in llm_response.lower(): # Check lowercased
                algorithm_desc_match = re.search(r'^(.*?)python', llm_response, re.DOTALL | re.IGNORECASE)
            elif 'import' in llm_response.lower():
                 algorithm_desc_match = re.search(r'^(.*?)import', llm_response, re.DOTALL | re.IGNORECASE)
            else:
                 algorithm_desc_match = re.search(r'^(.*?)def', llm_response, re.DOTALL | re.IGNORECASE)
            if algorithm_desc_match:
                algorithm_desc = algorithm_desc_match.group(1).strip()

        # Try to extract code block (from 'import' or 'def' up to 'return ...')
        # This regex tries to find the function definition robustly
        code_match = re.search(r"(import numpy as np\s*\n)?(import math\s*\n)?(def\s+" + re.escape(self.prompt_func_name) + r"\s*\(.*?\):.*?)(?=\n\S|\Z)", llm_response, re.DOTALL)
        # The positive lookahead (?=\n\S|\Z) tries to ensure it captures the whole function body
        # up to the next non-whitespace line or end of string.
        
        code_str = None
        if code_match:
            # Reconstruct the code, ensuring the return statement is complete.
            # The regex for code extraction needs to be robust.
            # A simpler way might be to look for "def function_name(...):" and then parse until logical end.
            # The original regex `r"import.*return"` or `r"def.*return"` can be greedy.
            
            # Try to find the function definition and extract it.
            # A more robust method would involve parsing, but regex is used here.
            # A common pattern is ```python ... ``` or the raw code.
            python_block_match = re.search(r"```python\n(.*?)```", llm_response, re.DOTALL)
            if python_block_match:
                code_str = python_block_match.group(1).strip()
            else: # Fallback to find "def function_name(...):"
                def_match = re.search(r"def\s+" + re.escape(self.prompt_func_name) + r"\s*\(.*?\):.*?(?:\n\s{4,}.*|\n\n|$)", llm_response, re.DOTALL)
                if def_match:
                    code_str = def_match.group(0).strip()
                else: # Last resort, try the original simpler regexes if others fail
                    code_block_candidates = re.findall(r"def\s+" + re.escape(self.prompt_func_name) + r".*?return.*?(?:\n\S|\Z)", llm_response, re.DOTALL)
                    if not code_block_candidates:
                        code_block_candidates = re.findall(r"import.*?def\s+" + re.escape(self.prompt_func_name) + r".*?return.*?(?:\n\S|\Z)", llm_response, re.DOTALL)
                    
                    if code_block_candidates:
                        code_str = code_block_candidates[0].strip() # Take the first likely block


        # If extraction fails after retries, algorithm_desc or code_str might be None.
        if algorithm_desc is None and self.debug_mode:
            print("Warning: Algorithm description not identified clearly.")
        if code_str is None and self.debug_mode:
            print("Warning: Code block not identified clearly.")
            # print(f"LLM Response was: \n{llm_response}")


        # Ensure the extracted code includes the full return statement if prompt_func_outputs is used.
        # This part of the original code `code_all = code + " " + ", ".join(s for s in self.prompt_func_outputs)`
        # assumes `code` ends just before the returned variables. This is fragile.
        # A better approach is to ensure the LLM generates the full function including the return statement.
        # The prompt already asks for "return ... outputs: self.joined_outputs".
        # So, the extracted code_str should ideally be complete.
        
        return code_str, algorithm_desc


    def _get_alg_from_llm(self, prompt_for_llm):
        """Helper to get algorithm and code from LLM with retries."""
        code_final = None
        algorithm_final = None
        
        for retry_count in range(3): # Max 3 attempts
            llm_response = self.interface_llm_algo_gen.get_response(prompt_for_llm)
            if llm_response:
                code_final, algorithm_final = self._extract_algorithm_and_code(llm_response)
                if code_final and algorithm_final: # Successful extraction
                    break 
            if self.debug_mode:
                print(f"Error: Algorithm or code not identified (attempt {retry_count + 1}). Retrying...")
            if retry_count < 2 : time.sleep(1) # Wait before retry, except for the last one

        return code_final, algorithm_final


    def initial_algorithm(self):
        ####################################################################
        # Get algorithm description and code for initial population        #
        ####################################################################
        prompt_content = self.get_prompt_initial_algorithm()
        if self.debug_mode:
            print("\n>>> Debug: Query to LLM for generating initial algorithm:\n", prompt_content)
        
        code_all, algorithm_desc = self._get_alg_from_llm(prompt_content)

        if self.debug_mode:
            if algorithm_desc: print("\n>>> Debug: Initial algorithm description:\n", algorithm_desc)
            if code_all: print("\n>>> Debug: Initial algorithm code:\n", code_all)
        return code_all, algorithm_desc

    def cross_algorithm(self, parent_algorithms_info, guiding_prompt_text):
        ####################################################################
        # Get new algorithm inspired by parents and a guiding prompt       #
        ####################################################################
        prompt_content = self.get_prompt_cross_algorithm(parent_algorithms_info, guiding_prompt_text)
        if self.debug_mode:
            print("\n>>> Debug: Query to LLM for crossover algorithm generation:\n", prompt_content)

        code_all, algorithm_desc = self._get_alg_from_llm(prompt_content)

        if self.debug_mode:
            if algorithm_desc: print("\n>>> Debug: Crossover algorithm description:\n", algorithm_desc)
            if code_all: print("\n>>> Debug: Crossover algorithm code:\n", code_all)
        return code_all, algorithm_desc

    def variation_algorithm(self, parent_algorithm_info, guiding_prompt_text):
        ####################################################################
        # Get new algorithm by varying a parent, guided by a prompt        #
        ####################################################################
        prompt_content = self.get_prompt_variation_algorithm(parent_algorithm_info, guiding_prompt_text)
        if self.debug_mode:
            print("\n>>> Debug: Query to LLM for variation algorithm generation:\n", prompt_content)

        code_all, algorithm_desc = self._get_alg_from_llm(prompt_content)

        if self.debug_mode:
            if algorithm_desc: print("\n>>> Debug: Variation algorithm description:\n", algorithm_desc)
            if code_all: print("\n>>> Debug: Variation algorithm code:\n", code_all)
        return code_all, algorithm_desc


####################################################################################
# Interface for EC (Algorithm Evolution)                                           #
####################################################################################
class InterfaceEC:
    def __init__(self, algo_pop_size, m_parents_algo, api_endpoint, api_key, llm_model,
                 debug_mode, problem_interface, select_method, num_parallel_jobs,
                 evaluation_timeout, use_numba_decorator):

        self.algo_pop_size = algo_pop_size  # Number of algorithms to generate in each step (e.g. per guiding prompt)
        self.m_parents_algo = m_parents_algo # Number of parent algorithms for crossover

        self.problem_eval_interface = problem_interface # PROBLEMCONST instance for evaluation
        # Prompts defining the problem structure (e.g., function name, inputs/outputs)
        problem_prompts_config = problem_interface.prompts 
        self.algo_generator = Evolution(api_endpoint, api_key, llm_model, debug_mode, problem_prompts_config)
        ...
        self.select_method = select_method # Parent selection for algorithms
        self.num_parallel_jobs = num_parallel_jobs
        self.evaluation_timeout = evaluation_timeout
        self.use_numba_decorator = use_numba_decorator

    def add_to_algorithm_population(self, algorithm_population, offspring_algorithm_dict):
        # Add a new offspring algorithm if its objective is not a duplicate.
        if offspring_algorithm_dict is None or offspring_algorithm_dict['objective'] is None:
            if self.debug: print("Attempted to add invalid/unevaluated algorithm. Skipping.")
            return False
            
        for ind_algo in algorithm_population:
            if ind_algo['objective'] == offspring_algorithm_dict['objective']: # Compare objectives
                # Could also compare code strings for true duplication if objectives can be non-unique
                if self.debug:
                    print("Duplicated algorithm objective, not adding to algorithm population.")
                return False
        algorithm_population.append(offspring_algorithm_dict)
        return True

    def check_duplicate_code(self, algorithm_population, new_code_string):
        # Check if the generated code string already exists in the population.
        if new_code_string is None: return True # Treat None code as duplicate to force regeneration
        for ind_algo in algorithm_population:
            if ind_algo['code'] == new_code_string:
                return True
        return False

    def _generate_single_offspring_algorithm(self, algo_pop, operator, guiding_prompt_text):
        # Initialize offspring algorithm dictionary
        offspring_algorithm = {
            'algorithm': None, # Description
            'code': None,
            'objective': None,
            'other_inf': None # Could store parent info or other metadata
        }
        selected_parents_info = None

        if operator == "initial":
            # Guiding_prompt_text is not used for initial algorithm generation via self.algo_generator.initial_algorithm()
            offspring_algorithm['code'], offspring_algorithm['algorithm'] = self.algo_generator.initial_algorithm()
        elif operator == "cross":
            selected_parents_info = self.select_method.parent_selection(algo_pop, self.m_parents_algo)
            if not selected_parents_info or len(selected_parents_info) < self.m_parents_algo:
                if self.debug: print("Not enough parents for algorithm crossover.")
                return None, None # Return None for parents and offspring
            offspring_algorithm['code'], offspring_algorithm['algorithm'] = self.algo_generator.cross_algorithm(
                selected_parents_info, guiding_prompt_text
            )
        elif operator == "variation":
            selected_parents_info = self.select_method.parent_selection(algo_pop, 1)
            if not selected_parents_info:
                if self.debug: print("No parent selected for algorithm variation.")
                return None, None # Return None for parents and offspring
            offspring_algorithm['code'], offspring_algorithm['algorithm'] = self.algo_generator.variation_algorithm(
                selected_parents_info[0], guiding_prompt_text
            )
        else:
            print(f"Algorithm evolution operator [{operator}] has not been implemented!")
            return None, None # Return None for parents and offspring
        
        return selected_parents_info, offspring_algorithm


    def get_offspring_algorithm_evaluated(self, algo_pop, operator, guiding_prompt_text):
        # Generates and evaluates a single offspring algorithm.
        parents_info = None
        offspring_dict = None

        try:
            # Generate algorithm (code + description)
            # Retry if generated code is duplicate (up to a limit)
            for _attempt in range(2): # Try once, then retry once if duplicate
                parents_info, offspring_dict_uneval = self._generate_single_offspring_algorithm(
                    algo_pop, operator, guiding_prompt_text
                )
                if offspring_dict_uneval and offspring_dict_uneval['code']:
                    if not self.check_duplicate_code(algo_pop, offspring_dict_uneval['code']):
                        break # Unique code found
                    if self.debug: print("Duplicate algorithm code generated, retrying generation...")
                elif offspring_dict_uneval is None or offspring_dict_uneval['code'] is None: # Generation failed
                     if self.debug: print(f"Algorithm generation failed for operator {operator}. No code produced.")
                     return parents_info, None # Propagate failure

            if offspring_dict_uneval is None or offspring_dict_uneval['code'] is None:
                if self.debug: print("Failed to generate a unique, valid algorithm after retries.")
                return parents_info, None

            code_to_evaluate = offspring_dict_uneval['code']

            # Optionally add Numba decorator
            if self.use_numba_decorator:
                # Add numba.jit to the generated function
                # First, ensure 'numba' and target function name are correctly handled by add_numba_decorator
                # For this, we need to parse the function name from code_to_evaluate
                match = re.search(r"def\s+(\w+)\s*\(", code_to_evaluate)
                if match:
                    function_name = match.group(1)
                    # The add_numba_decorator function itself handles importing numba
                    try:
                        # This utility is not part of the provided code, so commenting out its usage.
                        # If available, it would be:
                        # code_to_evaluate = add_numba_decorator(program=code_to_evaluate, function_name=function_name)
                        # For now, this step is skipped if the utility is not defined.
                        pass # Placeholder if add_numba_decorator is not available
                    except Exception as e_numba:
                        if self.debug: print(f"Failed to apply Numba decorator: {e_numba}")
                else:
                    if self.debug: print("Could not parse function name to apply Numba decorator.")
            
            # Evaluate the algorithm's code
            # Using ThreadPoolExecutor for timeout, though evaluate itself is likely blocking.
            # If evaluate is truly async or can be made so, this is useful.
            # Otherwise, a simple direct call is fine if evaluate handles its own timeouts or is quick.
            fitness = None
            with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
                future = executor.submit(self.problem_eval_interface.evaluate, code_to_evaluate)
                try:
                    fitness = future.result(timeout=self.evaluation_timeout)
                except concurrent.futures.TimeoutError:
                    if self.debug: print(f"Evaluation timed out for operator {operator}.")
                    future.cancel() # Attempt to cancel
                    fitness = None # Or a very bad fitness value
                except Exception as e_eval: # Catch other errors from evaluate
                    if self.debug: print(f"Exception during evaluation: {e_eval}")
                    fitness = None
            
            if fitness is not None:
                offspring_dict_uneval['objective'] = round(fitness, 5)
            else:
                offspring_dict_uneval['objective'] = None # Mark as unevaluated / failed

            offspring_dict = offspring_dict_uneval # Now it's evaluated (or marked failed)

        except Exception as e:
            if self.debug: print(f"Outer exception in get_offspring_algorithm_evaluated ({operator}): {e}")
            # Ensure offspring_dict is None or has None objective if major error
            if offspring_dict: offspring_dict['objective'] = None
            else: offspring_dict = {'algorithm': None, 'code': None, 'objective': None, 'other_inf': None}
            # parents_info might be partially set or None

        return parents_info, offspring_dict


    def get_evolved_algorithms(self, algo_pop, operator, guiding_prompt_text):
        # Generates self.algo_pop_size algorithms in parallel using the specified operator and guiding prompt.
        results_tuples = []
        try:
            results_tuples = Parallel(n_jobs=self.num_parallel_jobs, timeout=self.evaluation_timeout + 15)(
                delayed(self.get_offspring_algorithm_evaluated)(algo_pop, operator, guiding_prompt_text)
                for _ in range(self.algo_pop_size)
            )
        except Exception as e: # Catch timeout from Parallel itself
            if self.debug:
                print(f"Error in Parallel algorithm generation/evaluation ({operator}): {e}")
            print("Parallel algorithm processing may have timed out or failed.")

        time.sleep(1) # Small delay

        all_parent_algos_info = []
        all_new_offspring_algos = []

        for p_info, off_algo in results_tuples:
            if p_info: all_parent_algos_info.append(p_info)
            if off_algo and off_algo.get('objective') is not None: # Only add successfully evaluated offspring
                all_new_offspring_algos.append(off_algo)
            elif self.debug and off_algo:
                print(f">>> Debug: Offspring algorithm evaluation failed or was invalid: \n {off_algo}")
            elif self.debug and not off_algo:
                 print(">>> Debug: Offspring algorithm generation failed (returned None).")


        if self.debug:
            print(f">>> Debug: Generated {len(all_new_offspring_algos)} valid new algorithms for operator '{operator}'.")
        return all_parent_algos_info, all_new_offspring_algos


    def initial_algorithm_population_generation(self):
        # Generates an initial population of algorithms.
        # This will make self.algo_pop_size calls to generate initial algorithms in parallel.
        # The "guiding_prompt_text" is not applicable for the "initial" operator.
        _, population_of_algorithms = self.get_evolved_algorithms([], "initial", "")
        
        # Filter out any None or unevaluated algorithms
        valid_population = [
            algo for algo in population_of_algorithms 
            if algo and algo.get('objective') is not None
        ]
        if self.debug:
            print(f"Initial algorithm population generated {len(valid_population)} valid algorithms.")
        return valid_population

    def seed_algorithm_population_generation(self, seed_algorithms_data):
        # Generates a population from pre-defined seed algorithms, evaluating them in parallel.
        population_from_seeds = []

        # Prepare arguments for parallel evaluation
        codes_to_evaluate = [seed['code'] for seed in seed_algorithms_data]
        
        evaluated_fitness_list = []
        try:
            evaluated_fitness_list = Parallel(n_jobs=self.num_parallel_jobs, timeout=self.evaluation_timeout + 15)(
                delayed(self.problem_eval_interface.evaluate)(code) for code in codes_to_evaluate
            )
        except Exception as e:
            if self.debug: print(f"Error during parallel evaluation of seed algorithms: {e}")
            print("Seed algorithm evaluation may have failed or timed out.")
            # evaluated_fitness_list might be incomplete or empty

        for i, seed_data in enumerate(seed_algorithms_data):
            if i < len(evaluated_fitness_list) and evaluated_fitness_list[i] is not None:
                fitness = evaluated_fitness_list[i]
                seed_algo_dict = {
                    'algorithm': seed_data['algorithm'],
                    'code': seed_data['code'],
                    'objective': round(fitness, 5),
                    'other_inf': "seeded"
                }
                population_from_seeds.append(seed_algo_dict)
            else:
                if self.debug: print(f"Failed to evaluate seed algorithm: {seed_data.get('algorithm', 'Unknown')}")
        
        print(f"Initialization from seeds finished! Got {len(population_from_seeds)} successfully evaluated seed algorithms.")
        return population_from_seeds


class EOH:
    # Main Evolution of Heuristics class
    def __init__(self, paras, problem_obj, select_method, manage_method):
        self.problem_obj = problem_obj  # Problem instance (e.g., from PROBLEMCONST)
        self.select_method = select_method  # Parent selection method object
        self.manage_method = manage_method  # Population management method object
        ...
        self.pop_size_prompts_cross = 2 # How many new "crossover-style" prompts to generate per prompt-evo step
        self.pop_size_prompts_variation = 2 # How many new "variation-style" prompts to generate
        self.problem_type_for_prompt_evo = "minimization" # Or "maximization"
        self.m_parents_prompts = 2 # Number of parent prompts for crossover

        # Algorithm evolution settings
        self.algo_pop_size_per_prompt = paras.ec_pop_size  # Num algorithms generated per guiding prompt
        self.num_evolution_generations = paras.ec_n_pop    # Total generations for algorithm evolution
        self.m_parents_algo = paras.ec_m                   # Num parent algorithms for crossover

        # self.operators = paras.ec_operators # Not used in current EOH.run()
        # self.operator_weights = paras.ec_operator_weights # Not used

        self.debug_mode = paras.exp_debug_mode
        self.output_path = paras.exp_output_path
        self.num_parallel_jobs = paras.exp_n_proc
        self.evaluation_timeout = paras.eva_timeout
        self.use_numba = paras.eva_numba_decorator

        print("- EoH parameters loaded -")
        random.seed(2024) # Set random seed

    def add_to_population(self, population, offspring_list, type="algorithm"):
        # Helper to add offspring (prompts or algorithms) to their respective populations
        # Handles None offspring and checks for duplicates based on 'prompt' or 'objective'.
        added_count = 0
        for off in offspring_list:
            ...
            is_duplicate = False
            if type == "prompt":
                if off.get('prompt') is None: continue
                for ind in population:
                    if ind['prompt'] == off['prompt']:
                        is_duplicate = True
                        break
            else: # algorithm
                if off.get('objective') is None: continue # Skip unevaluated/failed algorithms
                for ind in population:
                    if ind.get('objective') == off['objective']: # Could also check code string
                        is_duplicate = True
                        break
            ...
            if not is_duplicate:
                ...
                print(f"Duplicated {type} result (based on {'prompt' if type=='prompt' else 'objective'}), not adding.")
        return added_count
    ...
    def run(self):
        ...
        interface_prompt_evo = InterfaceEC_Prompt(
            pop_size_prompts=1, # This is used as num_to_generate in get_evolved_prompts for 'cross'/'variation'
                                # If pop_size_prompts_cross/variation is meant to control this, adjust here.
                                # Current InterfaceEC_Prompt.get_evolved_prompts uses self.pop_size_prompts.
                                # Let's assume pop_size_prompts_cross/variation sets this.
            m_parents_prompts=self.m_parents_prompts,
            api_endpoint=self.api_endpoint, api_key=self.api_key, llm_model=self.llm_model,
            debug_mode=self.debug_mode, select_method=self.select_method,
            num_parallel_jobs=self.num_parallel_jobs, timeout=self.evaluation_timeout, # Prompt LLM calls can also timeout
            problem_type=self.problem_type_for_prompt_evo
        )
        # Configure pop_size_prompts for the interface_prompt_evo instance
        # This parameter seems to determine how many prompts are generated in parallel *per call* to get_evolved_prompts
        # if the operator is 'cross' or 'variation'.
        # Let's set it to a sensible value, e.g., self.pop_size_prompts_cross for now.
        # This needs clarification on how pop_size_prompts_cross/variation are intended to be used.
        # If one call to get_evolved_prompts(..., 'cross') should yield pop_size_prompts_cross prompts,
        # then interface_prompt_evo.pop_size_prompts should be set to that.
        # The original code's `interface_promt_cross = InterfaceEC_Prompt(self.pop_size_cross, ...)`
        # implies that pop_size_cross is this parameter.

        # Interface for Evolving Algorithms
        interface_algo_evo = InterfaceEC(
            algo_pop_size=self.algo_pop_size_per_prompt, m_parents_algo=self.m_parents_algo,
            api_endpoint=self.api_endpoint, api_key=self.api_key, llm_model=self.llm_model,
            debug_mode=self.debug_mode, problem_interface=self.problem_obj,
            select_method=self.select_method, num_parallel_jobs=self.num_parallel_jobs,
            evaluation_timeout=self.evaluation_timeout, use_numba_decorator=self.use_numba
        )

        # Initialize prompt populations
        print("Creating initial guiding prompts:")
        # For crossover-guiding prompts:
        interface_prompt_evo.pop_size_prompts = self.pop_size_prompts_cross # Configure for cross
        prompts_for_algo_crossover = interface_prompt_evo.initial_prompt_population_generation("initial_cross")
        prompts_for_algo_crossover = self.manage_method.population_management(prompts_for_algo_crossover, self.pop_size_prompts_cross)
        
        # For variation-guiding prompts:
        interface_prompt_evo.pop_size_prompts = self.pop_size_prompts_variation # Configure for variation
        prompts_for_algo_variation = interface_prompt_evo.initial_prompt_population_generation("initial_variation")
        prompts_for_algo_variation = self.manage_method.population_management(prompts_for_algo_variation, self.pop_size_prompts_variation)

        print("Initial guiding prompts created:")
        for p in prompts_for_algo_crossover: print(f"  Crossover Prompt: \"{p['prompt']}\" (Score: {p['objective']})")
        for p in prompts_for_algo_variation: print(f"  Variation Prompt: \"{p['prompt']}\" (Score: {p['objective']})")
        print("=======================================")

        # Initialize algorithm population
        print("Creating initial algorithm population:")
        algorithm_population = interface_algo_evo.initial_algorithm_population_generation()
        algorithm_population = self.manage_method.population_management(algorithm_population, self.algo_pop_size_per_prompt) # Manage to overall algo pop size

        print("Initial algorithm population created:")
        for i, off in enumerate(algorithm_population):
            # Objective is a value to minimize. Fitness for maximization would be 1/(1+obj) or -obj.
            # Assuming 'objective' is directly the value from problem.evaluate().
            print(f"  Algo {i}: Obj: {off['objective']:.4f}", end=" | ")
        print("\n=======================================")

        # Save initial algorithm population
        filename_init_algo_pop = os.path.join(self.output_path, "results", "pops", "population_generation_0.json")
        os.makedirs(os.path.dirname(filename_init_algo_pop), exist_ok=True)
        with open(filename_init_algo_pop, 'w') as f:
            json.dump(algorithm_population, f, indent=4)
        
        # Evolution loop for algorithms (and prompts)
        worst_objectives_history = [] # Tracks worst objective in algorithm population
        prompt_evo_delay_turn = 2
        prompt_evo_active_change_flag = 0 # Countdown after prompt evolution
        last_prompt_evo_generation = -1
        max_prompt_performance_samples_k = 4 # For heapq

        for gen_idx in range(self.num_evolution_generations):
            print(f"\n--- Generation {gen_idx + 1} / {self.num_evolution_generations} ---")

            if prompt_evo_active_change_flag > 0:
                prompt_evo_active_change_flag -= 1
                if prompt_evo_active_change_flag == 0: # Cooldown finished, manage and print new prompts
                    prompts_for_algo_crossover = self.manage_method.population_management(
                        prompts_for_algo_crossover, self.pop_size_prompts_cross
                    )
                    prompts_for_algo_variation = self.manage_method.population_management(
                        prompts_for_algo_variation, self.pop_size_prompts_variation
                    )
                    print("Updated Guiding Prompts after evolution:")
                    for p in prompts_for_algo_crossover: print(f"  Crossover Prompt: \"{p['prompt']}\" (Score: {p['objective']})")
                    for p in prompts_for_algo_variation: print(f"  Variation Prompt: \"{p['prompt']}\" (Score: {p['objective']})")

            # Check for stagnation to trigger prompt evolution
            if (len(worst_objectives_history) >= prompt_evo_delay_turn and
                worst_objectives_history[-1] == worst_objectives_history[-prompt_evo_delay_turn] and
                gen_idx - last_prompt_evo_generation > prompt_evo_delay_turn):
                
                print("Algorithm performance stagnated. Evolving guiding prompts...")
                last_prompt_evo_generation = gen_idx
                
                # Evolve crossover prompts
                interface_prompt_evo.pop_size_prompts = self.pop_size_prompts_cross
                _, new_cross_prompts_evo = interface_prompt_evo.get_evolved_prompts(prompts_for_algo_crossover, 'cross')
                self.add_to_population(prompts_for_algo_crossover, new_cross_prompts_evo, type="prompt")
                _, new_cross_prompts_var = interface_prompt_evo.get_evolved_prompts(prompts_for_algo_crossover, 'variation')
                self.add_to_population(prompts_for_algo_crossover, new_cross_prompts_var, type="prompt")
                for p in prompts_for_algo_crossover: p['objective'], p['number'] = 1e9, [] # Reset scores

                # Evolve variation prompts
                interface_prompt_evo.pop_size_prompts = self.pop_size_prompts_variation
                _, new_var_prompts_evo = interface_prompt_evo.get_evolved_prompts(prompts_for_algo_variation, 'cross')
                self.add_to_population(prompts_for_algo_variation, new_var_prompts_evo, type="prompt")
                _, new_var_prompts_var = interface_prompt_evo.get_evolved_prompts(prompts_for_algo_variation, 'variation')
                self.add_to_population(prompts_for_algo_variation, new_var_prompts_var, type="prompt")
                for p in prompts_for_algo_variation: p['objective'], p['number'] = 1e9, [] # Reset scores
                
                prompt_evo_active_change_flag = 2 # Cooldown period

            # Algorithm Evolution using current guiding prompts
            print("Evolving algorithms using current prompts:")
            # Using crossover-guiding prompts
            for i, current_prompt_dict in enumerate(prompts_for_algo_crossover):
                guiding_prompt = current_prompt_dict["prompt"]
                print(f"  Using Crossover Prompt {i+1}/{len(prompts_for_algo_crossover)}: \"{guiding_prompt[:50]}...\"")
                _, new_algorithms = interface_algo_evo.get_evolved_algorithms(
                    algorithm_population, "cross", guiding_prompt
                )
                self.add_to_population(algorithm_population, new_algorithms, type="algorithm")
                
                # Update prompt's performance score
                for algo in new_algorithms:
                    if algo['objective'] is not None:
                        # Store negative objective because heapq is a min-heap, we want to track best (smallest) objectives
                        if len(current_prompt_dict["number"]) < max_prompt_performance_samples_k:
                            heapq.heappush(current_prompt_dict["number"], algo['objective'])
                        elif algo['objective'] < current_prompt_dict["number"][0]: # If new is better than worst in heap
                            heapq.heapreplace(current_prompt_dict["number"], algo['objective'])
                if current_prompt_dict["number"]:
                    current_prompt_dict["objective"] = np.mean(current_prompt_dict["number"]) # Average of best objectives seen
                
                algorithm_population = self.manage_method.population_management(
                    algorithm_population, self.algo_pop_size_per_prompt * len(prompts_for_algo_crossover) # Manage to a larger temp pool
                )
            # Manage algorithm population after all crossover-prompt-guided evolutions
            algorithm_population = self.manage_method.population_management(algorithm_population, self.algo_pop_size_per_prompt) # Final size for this stage

            # Using variation-guiding prompts
            for i, current_prompt_dict in enumerate(prompts_for_algo_variation):
                guiding_prompt = current_prompt_dict["prompt"]
                print(f"  Using Variation Prompt {i+1}/{len(prompts_for_algo_variation)}: \"{guiding_prompt[:50]}...\"")
                _, new_algorithms = interface_algo_evo.get_evolved_algorithms(
                    algorithm_population, "variation", guiding_prompt
                )
                self.add_to_population(algorithm_population, new_algorithms, type="algorithm")

                # Update prompt's performance score (similar logic as above)
                for algo in new_algorithms:
                     if algo['objective'] is not None:
                        if len(current_prompt_dict["number"]) < max_prompt_performance_samples_k:
                            heapq.heappush(current_prompt_dict["number"], algo['objective'])
                        elif algo['objective'] < current_prompt_dict["number"][0]:
                            heapq.heapreplace(current_prompt_dict["number"], algo['objective'])
                if current_prompt_dict["number"]:
                    current_prompt_dict["objective"] = np.mean(current_prompt_dict["number"])

                algorithm_population = self.manage_method.population_management(
                    algorithm_population, self.algo_pop_size_per_prompt * len(prompts_for_algo_variation)
                )
            # Manage algorithm population after all variation-prompt-guided evolutions
            algorithm_population = self.manage_method.population_management(algorithm_population, self.algo_pop_size_per_prompt)


            # Save current algorithm population
            filename_pop = os.path.join(self.output_path, "results", "pops", f"population_generation_{gen_idx + 1}.json")
            with open(filename_pop, 'w') as f:
                json.dump(algorithm_population, f, indent=4)

            # Save best algorithm of current population
            if algorithm_population: # Ensure population is not empty
                best_algo = sorted(algorithm_population, key=lambda x: x['objective'])[0]
                filename_best = os.path.join(self.output_path, "results", "pops_best", f"population_generation_{gen_idx + 1}.json")
                os.makedirs(os.path.dirname(filename_best), exist_ok=True)
                with open(filename_best, 'w') as f:
                    json.dump(best_algo, f, indent=4)
                worst_objectives_history.append(algorithm_population[-1]['objective']) # Assuming sorted by manage_method
            else: # Population might be empty if all generations failed
                print("Warning: Algorithm population is empty at end of generation.")
                worst_objectives_history.append(float('inf')) # Add a very bad objective if empty

            time_elapsed_minutes = (time.time() - time_start) / 60
            print(f"--- Generation {gen_idx + 1} finished. Time Cost: {time_elapsed_minutes:.1f} m")
            print("Current Algorithm Population Objectives (best to worst): ", end=" ")
            for algo_ind in algorithm_population:
                print(f"{algo_ind['objective']:.4f}", end=" ")
            print()


class Methods:
    # Maps method names (strings) to corresponding selection and management functions.
    def __init__(self, paras, problem_obj):
        self.paras = paras
        self.problem_obj = problem_obj # problem_obj is the instantiated problem (e.g. PROBLEMCONST)
        
        # Selection method mapping
        if paras.selection == "prob_rank": self.select_method = prob_rank
        elif paras.selection == "equal": self.select_method = equal
        elif paras.selection == 'roulette_wheel': self.select_method = roulette_wheel
        elif paras.selection == 'tournament': self.select_method = tournament
        else:
            print(f"Selection method '{paras.selection}' has not been implemented!")
            sys.exit(1)

        # Management method mapping
        if paras.management == "pop_greedy": self.manage_method = pop_greedy
        elif paras.management == 'ls_greedy': self.manage_method = ls_greedy
        elif paras.management == 'ls_sa': self.manage_method = ls_sa
        else:
            print(f"Management method '{paras.management}' has not been implemented!")
            sys.exit(1)

    def get_method_runner(self):
        # Returns an instance of the chosen evolutionary method runner (e.g., EOH)
        if self.paras.method == "eoh":
            return EOH(self.paras, self.problem_obj, self.select_method, self.manage_method)
        # elif self.paras.method == "ael":
            # return AEL(...) # Example for another method
        else:
...
class EVOL:
    # Main orchestrator for the evolutionary process
    def __init__(self, paras):
        print("-----------------------------------------")
        print("---              Start EoH            ---")
        print("-----------------------------------------")
        ...
        print("- Output folder checked/created -")

        self.paras = paras
        print("- Parameters loaded -")

        # Set a random seed for global operations if any (already in EOH for its specific parts)
        random.seed(2024) # Consistent global seed

    def run(self):
        # Instantiate the problem generator/handler
        problem_generator = Probs(self.paras)
        problem_instance = problem_generator.get_problem() # This is the PROBLEMCONST instance

        # Instantiate the method mapper/factory
        method_factory = Methods(self.paras, problem_instance)
        # Get the specific evolutionary method runner (e.g., EOH instance)
        evolution_runner = method_factory.get_method_runner()

        # Run the evolutionary process
        evolution_runner.run()
...
if __name__ == '__main__':
    ...
    # Set parameters
    # Note: Ensure llm_api_endpoint, llm_api_key, and llm_model are correctly set.
    # If using 'bp_online', ensure GetData is configured to load bin packing instances.
    # If using 'tsp_construct', ensure PROBLEMCONST and GetPrompts are suitable for TSP.
    # The current GetPrompts is for bin packing scoring.
    paras_config.set_paras(
        method="eoh",  # ['ael','eoh']
        problem="bp_online",  # ['tsp_construct', 'bp_online']
        # IMPORTANT: Replace with your actual LLM API details
        llm_api_endpoint=os.getenv("LLM_API_ENDPOINT", "your_llm_endpoint"),
        llm_api_key=os.getenv("LLM_API_KEY", "your_api_key"),
        llm_model=os.getenv("LLM_MODEL", "gpt-4o-mini"), # Or other compatible model
        ec_pop_size=4,  # Number of algorithms generated by each guiding prompt in parallel
        ec_n_pop=20,    # Number of generations for algorithm evolution
        exp_n_proc=4,   # Number of parallel processes for evaluations / LLM calls
        exp_debug_mode=False,
        exp_output_path="./EOH_Results" # Example output path
    )

    # Initialize and run the evolution process
    evolution_process = EVOL(paras_config)
