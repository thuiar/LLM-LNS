...
class Paras:
    ...
    def set_paras(self, *args, **kwargs):
        #################################
        ### Set multithreading, population strategy, and evaluation
        #################################
...
class GetPrompts:
    ...
    def __init__(self):
        ...
        # Prompt function name: select_neighborhood
    ...
    def get_other_inf(self):
        #################################
        ### Get other descriptions for the prompt
        #################################
...
class GetData:
    ########################################################
    ### Given an LP path, generate problem instances by parsing .lp files.
    ########################################################
    def generate_instances(self, lp_path): #'./test'
        ...
        for f in sample_files:
            ...
            # site[i][j]: index of the j-th decision variable in the i-th constraint
            ...
            # coefficient[i]: objective function coefficient for the i-th decision variable
            n = model.NumVars
            m = model.NumConstrs
            k = []
            site = []
...
class PROBLEMCONST:
    ...
    def __init__(self) -> None:
        ...
        # Call GetData() to get problem instances
        getData = GetData()
    ...
    def Gurobi_solver(self, n, m, k, site, value, constraint, constraint_type, coefficient, time_limit, obj_type, lower_bound, upper_bound, value_type, now_sol, now_col):
        ...
        - site: site[i][j] indicates which decision variable is the j-th variable in the i-th constraint.
        ...
        - coefficient: coefficient[i] indicates the objective function coefficient corresponding to the i-th decision variable.
    ...
    def run_with_timeout(self, time_limit, func, *args, **kwargs):
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            ...
            try:
                # Wait for the result, up to time_limit seconds
    ...
    def greedy(self, eva):
        ###############################################################################
        ### Use a greedy-like approach; in each step, select the next point via eva.select_neighborhood
        ### Run on multiple instances, returning the average of their results.
        ###############################################################################
        results = []
        try:
            num = 0
    ...
    def evaluate(self, code_string):
        ...
        ### Key question: The current strategy is a string, how to turn it into an executable code block?
        ###############################################################################
        try:
            ...
            with warnings.catch_warnings():
                ...
                # Use the exec function to execute code_string in the namespace of the heuristic_module.
                # exec can dynamically execute code in string format and store the results in the specified namespace.
                exec(code_string, heuristic_module.__dict__)

                # Add the newly created module to sys.modules so it can be imported like a normal module in the program. sys.modules is a dictionary that stores all imported modules.
                ...
                # Call a method self.greedy from the class, passing heuristic_module as a parameter.
            ...
            # Return None if an exception occurs, indicating that the code execution failed or the fitness value could not be successfully calculated.
...
class Probs:
    ###########################################
    ### Read problem instances or call PROBLEMCONST() to create them
    ###########################################
...
class InterfaceAPI:
    ...
    def get_response(self, prompt_content):
        ...
        while True:
            n_trial += 1
            # Check if the current attempt count exceeds the maximum allowed self.n_trial (5 times). If exceeded, return the current response (possibly None) and exit the function.
            if n_trial > self.n_trial:
                return response
            try:
                # Create an HTTPS connection to the API endpoint.
                conn = http.client.HTTPSConnection(self.api_endpoint)
                # Send a request to the /v1/chat/completions endpoint using the POST method, passing the request payload and headers.
...
class InterfaceLLM:
    ...
    def __init__(self, api_endpoint, api_key, model_LLM, debug_mode):
        ...
        # Warn if default settings are not changed
        if self.api_key == None or self.api_endpoint ==None or self.api_key == 'xxx' or self.api_endpoint == 'xxx':
            ...
            exit()
        # Create an instance of the InterfaceAPI class and assign it to self.interface_llm. InterfaceAPI is a class defined above for actually handling API requests.
        ...
        # Check if the response is None, meaning the API request failed or was misconfigured.
        if res == None:
    ...
    def get_response(self, prompt_content):
        ##############################################################################
        # Define a method get_response, used to get the LLM's response to given content.
...
#######################################
#######################################
### Outer Layer! Seems everything is ready? Time to prepare for evolution!
#######################################
#######################################
class Evolution_Prompt:
    ...
    def __init__(self, api_endpoint, api_key, model_LLM, debug_mode, problem_type, **kwargs):
        ...
        # Use the defined InterfaceLLM to set up the LLM; subsequently, its get_response(self, prompt_content) function can be used for communication.
    ...
    def get_prompt_cross(self,prompts_indivs):
        ...
        # Combine the ideas and corresponding code of the algorithms in indivs, forming statements like "Algorithm No. 1's idea and code are..., Algorithm No. 2's idea and code are...".
        prompt_indiv = ""
        ...
        # 1. Describe the task
        # 2. Tell the LLM how many algorithms we are providing and their characteristics (combining with prompt_indiv)
        # 3. Make a request for the LLM to create an algorithm completely different from the previously given ones
        # 4. Instruct the LLM to first describe its new algorithm and main steps in one sentence, and the description must be in parentheses.
        # 5. Next, implement it in Python as a function named self.prompt_func_name
        # 6. Tell the LLM how many inputs and outputs this function has, and what inputs and outputs (corresponding to the processed self.prompt_func_inputs and self.prompt_func_outputs)
...
+"\n"+"Do not give additional explanations!!! Just one sentences."
    ...
    def get_prompt_variation(self,prompts_indivs):
        ##################################################
        ### Prompt to modify a heuristic for performance improvement
        ##################################################
        
        # 1. Describe the task
        # 2. Tell the LLM that we are providing one algorithm, describing its idea and code.
        # 3. Request the LLM to create a new algorithm, different in form but a modified version of the provided one.
        # 4. Instruct the LLM to first describe its new algorithm and main steps in one sentence, and the description must be in parentheses.
        # 5. Next, implement it in Python as a function named self.prompt_func_name
        # 6. Tell the LLM how many inputs and outputs this function has, and what inputs and outputs (corresponding to the processed self.prompt_func_inputs and self.prompt_func_outputs)
        # 7. Describe some properties of the input and output data.
...
+"\n"+"Do not give additional explanations!!! Just one sentences."
    ...
    def cross(self,parents):
        ...
        # Get a prompt for the LLM to help create an algorithm as different as possible from parent heuristics
        prompt_content = self.get_prompt_cross(parents)
        #print("???", prompt_content)
        response = self.interface_llm.get_response(prompt_content)

        # In debug mode, output the prompt for the LLM to help create an algorithm as different as possible from parent heuristics
        if self.debug_mode:
...
#######################################
#######################################
### Prompt: Communication and algorithm processing are ready, let's start!
#######################################
#######################################
class InterfaceEC_Prompt:
    def __init__(self, pop_size, m, api_endpoint, api_key, llm_model, debug_mode, select,n_p,timeout, problem_type, **kwargs):
        ...
        self.evol = Evolution_Prompt(api_endpoint, api_key, llm_model, debug_mode, problem_type , **kwargs)  # Evolution type, including i1, e1, e2, m1, m2, which can be used for algorithm evolution
        self.m = m                                  # Number of parent algorithms for prompt cross operation
        self.debug = debug_mode                     # Whether debug mode is enabled

        # If debug mode is not enabled, warnings are not displayed
        if not self.debug:
    ...
    # Prerequisite: the new individual's objective value is not duplicated among other individuals in the population
    ...
    def extract_first_quoted_string(self, text):
        # Use regex to match content in the first double quotes
    ...
    # Used to generate offspring individuals based on specified evolution operators
    def _get_alg(self,pop,operator):
        #print("Begin: 3")
        # Initialize offspring: create a dictionary offspring
        offspring = {
            'prompt': None,
            'objective': None,
            'number': None
    ...
    def population_generation(self, initial_type):
        ...
        n_create = 1
        # Create an empty list to store the generated initial population individuals
        population = []
...
#######################################
#######################################
### Inner Layer! Seems everything is ready? Time to prepare for evolution!
#######################################
####################################### 
...
class Evolution:
    ...
    def get_prompt_initial(self):
        ...
        # First, describe the task, then what the LLM needs to do:
        # 1. First, describe your new algorithm and main steps in one sentence. The description must be inside a brace.
        # 2. Next, implement it in Python as a function named self.prompt_func_name
        # 3. Tell the LLM how many inputs and outputs this function has, and what inputs and outputs (corresponding to the processed self.prompt_func_inputs and self.prompt_func_outputs)
...
+self.prompt_other_inf+"\n"+"Do not give additional explanations."
    ...
    def _get_alg(self,prompt_content):
        # Get response to given prompt_content via LLM interface.
        #print("QwQ~!")
        response = self.interface_llm.get_response(prompt_content)
        ...
        # Use regex re.findall(r"\{(.*)\}", response, re.DOTALL) to try extracting algorithm description enclosed in curly braces {} from the response.
        # The re.DOTALL option allows regex to match newline characters.
        algorithm = re.findall(r"\{(.*)\}", response, re.DOTALL)
        # If no algorithm description found in curly braces, use alternative patterns.
        if len(algorithm) == 0:
        ...
        # Try to use regex re.findall(r"import.*return", response, re.DOTALL) to extract the code part, which starts from 'import' and ends at 'return'.
        ...
        while (len(algorithm) == 0 or len(code) == 0):
            ...
            # Call get_response again to get a new response, and repeat attempts to extract algorithm description and code.
            response = self.interface_llm.get_response(prompt_content)
            algorithm = re.findall(r"\{(.*)\}", response, re.DOTALL)
            if len(algorithm) == 0:
                if 'python' in response:
            ...
            # If retry count exceeds 3 (n_retry > 3), exit the loop.
            if n_retry > 3:
                break
            n_retry += 1

        # Assuming algorithm description and code are successfully extracted, extract them from the list (i.e., take only the first match).
        algorithm = algorithm[0]
        code = code[0] 

        # The extracted code goes up to 'return'; we complete the rest. Connect the extracted code with the output variables (stored in self.prompt_func_outputs) to form a complete code string.
    ...
    def initial(self):
        ...
        # Get a prompt for the LLM to help create the initial population
        prompt_content = self.get_prompt_initial()

        # In debug mode, output the prompt for the LLM to help create the initial population
        if self.debug_mode:
            print("\n >>> check prompt for creating algorithm using [ i1 ] : \n", prompt_content )
            print(">>> Press 'Enter' to continue")
            
        #print("QAQ~!")
        # Call _get_alg, feed the prompt to the LLM, and split the returned text into code and algorithm description.
        [code_all, algorithm] = self._get_alg(prompt_content)

        # In debug mode, the returned text is split into code and algorithm description.
        if self.debug_mode:
...
### This function's purpose is to insert an 'import package_name as as_name' statement into the code, if the package is not already imported.
############################################################################################################
def add_import_package_statement(program: str, package_name: str, as_name=None, *, check_imported=True) -> str:
    ...
    # If check_imported is True, traverse each node in the AST to check if an import statement for the specified package already exists.
    if check_imported:
    ...
    # Create a new import node. Use ast.Import to create a new import statement node, with package_name as the package name and as_name as the alias (if any).
...
############################################################################################################
### Used to add NumPy's @numba.jit(nopython=True) decorator to given Python code.
...
def _add_numba_decorator(
    ...
    # Traverse each node of the AST to check if an 'import numba' statement already exists.
    ...
    for node in ast.walk(tree):
        # Use ast.walk(tree) to traverse all nodes of the AST, finding function definitions that match the function_name.
    ...
    # Use ast.unparse(tree) to convert the modified AST back into a Python code string and return the string.
...
############################################################################################################
# Used to insert a statement to set the random seed np.random.seed(...) in the specified Python code.
# If the code does not yet import the numpy module (i.e., 'import numpy as np' statement), this function first adds that import statement.
def add_np_random_seed_below_numpy_import(program: str, seed: int = 2024) -> str:
    ...
    for node in tree.body:
        ...
        if isinstance(node, ast.Import) and any(alias.name == 'numpy' and alias.asname == 'np' for alias in node.names):
            ...
            # Insert the 'np.random.seed(seed)' statement after the found 'import numpy as np' statement. This is achieved by creating a new AST node representing a call to the np.random.seed function.
    ...
    # If the 'import numpy as np' statement is not found in the code, raise a ValueError exception. This step ensures that numpy is imported before inserting 'np.random.seed(seed)'.
    if not found_numpy_import:
        raise ValueError("No 'import numpy as np' found in the code.")
    # Use the ast.unparse method to convert the modified syntax tree back into a Python code string and return it.
    modified_code = ast.unparse(tree)
    return modified_code

# Used to add an 'np.random.seed(seed)' statement inside a specified Python function to set the random number generator's seed.
...
def add_numpy_random_seed_to_func(program: str, func_name: str, seed: int = 2024) -> str:
    ...
    # Insert the new 'np.random.seed(seed)' statement at the beginning of the target function body.
    for node in tree.body:
...
############################################################################################################
### Replace division operator (/) in Python code with a custom protected division function _protected_div, and optionally accelerate computation using the Numba library.
############################################################################################################
...
class _CustomDivisionTransformer(ast.NodeTransformer):
    # Accepts a parameter custom_divide_func_name, which is the name of the custom function to replace the division operator. Here, the name is _protected_div.
    def __init__(self, custom_divide_func_name: str):
        super().__init__()
        self._custom_div_func = custom_divide_func_name

    # Used to visit all binary operator nodes. If a division operator (/) is detected, replace it with the custom function.
...
def replace_div_with_protected_div(code_str: str, delta=1e-5, numba_accelerate=False) -> Tuple[str, str]:
    # Define protected division function _protected_div
    protected_div_str = f'''
def _protected_div(x, y, delta={delta}):
    ...
    # Create _CustomDivisionTransformer instance and traverse the AST, finding and replacing division operations.
    transformer = _CustomDivisionTransformer('_protected_div')
    modified_tree = transformer.visit(tree)

    # Convert the modified AST back into a code string, here returning the replaced code and the protected division function definition together.
...
#######################################
#######################################
### Communication and algorithm processing are ready, let's start!
#######################################
#######################################
class InterfaceEC:
    def __init__(self, pop_size, m, api_endpoint, api_key, llm_model, debug_mode, interface_prob, select,n_p,timeout,use_numba,**kwargs):
        ...
        # Set information required by LLM
        self.pop_size = pop_size                    # Define population size
        self.interface_eval = interface_prob        # PROBLEMCONST() type, capable of calling the evaluate function to assess algorithm code.
        prompts = interface_prob.prompts            # Prompt for problem description, input/output information, which can be used to generate subsequent prompts.
        self.evol = Evolution(api_endpoint, api_key, llm_model, debug_mode,prompts, **kwargs)  # Evolution type, including i1, e1, e2, m1, m2, which can be used for algorithm evolution.
    ...
    def _get_alg(self,pop,operator, prompt):
        ...
        # Generate an algorithm dissimilar to its parents.
        elif operator == "cross":
            parents = self.select.parent_selection(pop,self.m)
            [offspring['code'],offspring['algorithm']] = self.evol.cross(parents, prompt)
        # Generate a new algorithm by improving the current one.
        elif operator == "variation":
            parents = self.select.parent_selection(pop,1)
    ...
    def population_generation_seed(self,seeds,n_p):
        ...
        population = []
        # Evaluate the code of each seed using the self.interface_eval.evaluate method and calculate its fitness.
...
class EOH:
    ...
    def __init__(self, paras, problem, select, manage, **kwargs):
        ...
        self.api_key = paras.llm_api_key            # Private API key
        ...
        self.n_pop = paras.ec_n_pop        # Number of populations/iterations

        self.operators = paras.ec_operators   # Define number of operators, default is e1, e2, m1, m2
        ...
        self.output_path = paras.exp_output_path             # Population result save path
    ...
    # Add newly generated offspring to the population; if debug mode is enabled, compare with current population individuals one by one for redundancy
    def add2pop(self, population, offspring):
    ...
    def run(self):
        ...
        for pop in range(n_start, self.n_pop):  
            ...
            # First, let's look at the crossover operation
            for i in range(len(cross_operators)):
                ...
                for off in offsprings:
                    ...
                    if len(cross_operators[i]["number"]) < max_k:
                        ...
                        # If the heap is full and the current element is smaller than the heap top, replace the heap top element
                ...
                # With new generation added, population size might exceed; manage population to keep size at most pop_size
                size_act = min(len(population), self.pop_size)
                population = self.manage.population_management(population, size_act)
                print(f"Cross {i + 1}, objective: {cross_operators[i]['objective']}", end = "|")
                print()
            
            # Next, let's look at the variation operation
            for i in range(len(cross_operators)):
            ...
            for i in range(n_op):
                ...
                # If random number is less than weight (weight range is 0 to 1), then execute
            ...
            # Save population to file, each generation in its own file
            filename = self.output_path + "/results/pops/population_generation_" + str(pop + 1) + ".json"
            with open(filename, 'w') as f:
                json.dump(population, f, indent=5)

            # Save best individual of population to file, each generation in its own file
...
class Methods:
    # Set parent selection and population management methods by mapping strings to function methods, which is quite clever.
