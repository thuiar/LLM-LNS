...
class Paras:
    def __init__(self):
        ...
        ### General settings  ###
        #####################
        self.method = 'eoh'                # Method chosen to use
        self.problem = 'milp_construct'     # Problem chosen to solve
        self.selection = None              # Method chosen for individual selection (how individuals are selected from the population for evolution)
        self.management = None             # Method chosen for population management
        ...
        self.exp_output_path = "./IS_ACP/"  # default folder for ael outputs
        self.exp_n_proc = 1
    ...
    def set_ec(self):
        ###################################################################
        ### Set population management strategy, parent selection strategy, ###
        ### and evolution strategies with corresponding weights           ###
        ###################################################################
    ...
    def set_paras(self, *args, **kwargs):
        #####################################################
        ### Set multi-threading, population strategies, and evaluation ###
        #####################################################
...
#######################################
#######################################
### All set! Let's configure the prompts. ###
#######################################
#######################################

def create_folders(results_path):
    #####################################################
    ### Create result folders, and within them, create history, populations, and best populations folders ###
    #####################################################
    folder_path = os.path.join(results_path, "results")

    # Check if the folder already exists
    if not os.path.exists(folder_path):
        # Remove the existing folder and its contents (original code had this commented out)
...
class GetPrompts:
    ...
    def __init__(self):
        ...
        # Name of the prompt function: select_neighborhood
        self.prompt_func_name = "select_neighborhood"
        # Inputs of the prompt function - problem parameters
        self.prompt_func_inputs = ["n", "m", "k", "site", "value", "constraint", "initial_solution", "current_solution", "objective_coefficient"]
        # Output of the prompt function - neighborhood score
        self.prompt_func_outputs = ["neighbor_score"]
        # Description of the prompt function's inputs and outputs
        ...
        This corresponds to the Independent Set MILP problem, where all decision variables are binary (0-1 variables), and all constraints are in the form of LHS \u2264 RHS.\
        ...
        # Other descriptions for the prompt
...
#######################################
#######################################
### Let's start creating problem instances! ###
#######################################
#######################################

class GetData:
    ########################################################
    ### Parse MILP instances from .lp files, extracting problem parameters. ###
    ########################################################
    def generate_instances(self, lp_path): #'./test'
        sample_files = [str(path) for path in Path(lp_path).glob("*.lp")]
        instance_data = []
        for f in sample_files: # For each instance, generate randomly
            model = read(f)
            value_to_num = {}
            value_to_type = {}
            value_num = 0
            ...
            # constraint[i]: RHS value of the i-th constraint
            ...
            for i in range(n):
                ...
                if(value_type[i] == 'B'):
                    new_value_type[i] = 0 # Binary
                elif(value_type[i] == 'C'):
                    new_value_type[i] = 1 # Continuous
                else:
                    new_value_type[i] = 2 # Integer
...
class PROBLEMCONST:
    ###########################################
    ### Create new MILP problem instances ###
    ###########################################
    def __init__(self) -> None:
        self.path = "./IS_easy_instance/LP"
        self.set_time = 100
    ...
    def Gurobi_solver(self, n, m, k, site, value, constraint, constraint_type, coefficient, time_limit, obj_type, lower_bound, upper_bound, value_type, now_sol, now_col):
        ...
        Solves a given problem instance using the Gurobi solver.
        ...
        - constraint: constraint[i] indicates the RHS value of the i-th constraint.
        - constraint_type: constraint_type[i] indicates the type of the i-th constraint (1 for <=, 2 for >=, 3 for ==).
        - coefficient: coefficient[i] indicates the objective function coefficient for the i-th decision variable.
        - time_limit: Maximum solving time.
        - obj_type: Whether the problem is a maximization or minimization problem.
        '''
        # Get start time
            ...
            if(flag == 1):
                ...
                if(constraint_type[i] == 1):
                    if(constr > constraint[i]):
                        print("Constraint violated (<=): ", constr, constraint[i])
                        print(now_col)
                else:
                    if(constr < constraint[i]):
                        print("Constraint violated (>=): ", constr, constraint[i])
    ...
    def greedy(self, eva):
        ###############################################################################
        ### Use a greedy-like method, selecting the next point via eva.select_next_node at each step ###
        ### Run multiple instances, returning the average of their results as the final result ###
        ###############################################################################
        results = []
        try:
            num = 0
            for now_instance_data in self.instance_data:
                ...
                #print("QAQ!")
            # Generate self.pop_size offspring individuals. Results are stored in the results list,
            # each element being a (p, off) tuple, where p is the parent individual and off is the generated offspring.
    ...
    def evaluate(self, code_string):
        ...
        try:
            ...
            with warnings.catch_warnings():
                ...
                # will be ignored and not displayed to the user.
                warnings.simplefilter("ignore")

                # Create a new module object named "heuristic_module". types.ModuleType creates a new empty module,
                # similar to a container for the code to be executed.
                ...
                # Add the newly created module to sys.modules, making it importable like a regular module in the program.
                ...
                # Call the self.greedy method from the class, passing heuristic_module as a parameter.
            ...
            # Return None if an exception occurs, indicating that code execution failed or fitness could not be calculated successfully.
...
class Probs:
    ###########################################
    ### Read problem instances or create them by calling PROBLEMCONST() ###
    ###########################################
...
#######################################
#######################################
### Finally, the module for communicating with LLM! ###
#######################################
#######################################
class InterfaceAPI:
    #######################################
    ...
    def __init__(self, api_endpoint, api_key, model_LLM, debug_mode):
        ...
        self.n_trial = 5                 # Indicates a maximum of 5 attempts to get a response

    def get_response(self, prompt_content):
        # Create a JSON formatted string payload_explanation, containing the model name and message content.
        ...
        n_trial = 1       # Initialize attempt count n_trial to 1, indicating the first attempt is starting.

        # Start an infinite loop to repeatedly attempt to get API response until successful or max attempts reached.
        while True:
            n_trial += 1
            # Check if the current attempt count exceeds the maximum allowed attempts self.n_trial (5 times).
            # If exceeded, return the current response (which might be None) and exit the function.
            ...
            try:
                ...
                # server_b_url = "http://43.134.189.32:5000/openai" (Original commented out code)
                # response = requests.post(server_b_url, json={"prompt": prompt_content}).json()['response'] (Original commented out code)
                break
            except Exception: # Catch any exception during API call
...
class InterfaceLLM:
    ...
    def __init__(self, api_endpoint, api_key, model_LLM, debug_mode):
        ...
        if self.api_key is None or self.api_endpoint is None or self.api_key == 'xxx' or self.api_endpoint == 'xxx':
            ...
            exit()
        # Create an instance of the InterfaceAPI class and assign it to self.interface_llm.
        # InterfaceAPI is a class defined above for handling actual API requests.
        ...
        # Call the get_response method of the InterfaceAPI instance, sending a simple request "1+1=?"
        # to test API connection and configuration.
        res = self.interface_llm.get_response("1+1=?")

        # Check if the response is None, indicating API request failure or misconfiguration.
        if res is None:
    ...
    def get_response(self, prompt_content):
        ##############################################################################
        # Define a method get_response, used to obtain LLM's response to given content. #
        # It accepts one parameter prompt_content, representing the user-provided prompt content. #
        ##############################################################################

        # Call the get_response method of the InterfaceAPI instance, sending the prompt content and getting the response.
...
#######################################
#######################################
### Outer Layer! All seems ready? Let's prepare for evolution! ###
#######################################
#######################################
class Evolution_Prompt:
    ...
    def __init__(self, api_endpoint, api_key, model_LLM, debug_mode, problem_type, **kwargs):
        ...
        # Set up LLM using the defined InterfaceLLM; now its get_response(self, prompt_content) function can be used for communication.
    ...
    def get_prompt_cross(self, prompts_indivs):
        ...
        # Combine the ideas and corresponding code of algorithms in indivs to form statements like
        # "Algorithm No. 1 and its code are..., Algorithm No. 2 and its code are..."
        prompt_indiv = ""
        ...
        # 1. Describe the task
        # 2. Tell LLM how many algorithms we're providing and how they perform (combined with prompt_indiv)
        # 3. Request LLM to create a new algorithm completely different from the given ones
        ...
        # 6. Tell LLM how many inputs and outputs this function has, and what they are (corresponding to processed self.prompt_func_inputs and self.prompt_func_outputs)
        # 7. Describe some properties of the input and output data.
        # 8. Describe some other supplementary properties.
        # 9. Finally, emphasize no other explanations, just strictly follow the requirements for output.
...
+"\n"+"Do not give additional explanations!!! Just one sentences."
    ...
    def get_prompt_variation(self, prompts_indivs):
        ...
        # 1. Describe the task
        # 2. Tell LLM that we're providing one algorithm, its idea and code
        # 3. Request LLM to create a new algorithm, different in form but a modified version of the provided one
        # 4. Instruct LLM to first describe its new algorithm and main steps in one sentence, enclosed in parentheses.
        # 5. Next, implement it in Python as a function named self.prompt_func_name
        # 6. Tell LLM how many inputs and outputs this function has, and what they are (corresponding to processed self.prompt_func_inputs and self.prompt_func_outputs)
        # 7. Describe some properties of the input and output data.
...
+"\n"+"Do not give additional explanations!!! Just one sentences."
    ...
    def cross(self, parents):
        #print("Begin: 4")
        #####################################################
        ### Obtain algorithm descriptions and code for new heuristics that are as different as possible from parent heuristics ###
        #####################################################
    ...
    def variation(self, parents):
        ###########################################################
        ### Obtain algorithm descriptions and code for new heuristics that modify the current heuristic to improve performance ###
        ###########################################################
...
#######################################
#######################################
### Inner Layer! All seems ready? Let's prepare for evolution! ###
#######################################
#######################################
class InterfaceEC_Prompt:
    def __init__(self, pop_size, m, api_endpoint, api_key, llm_model, debug_mode, select, n_p, timeout, problem_type, **kwargs):
        ...
        # `Evolution_Prompt` type, used for prompt evolution with operators like initial, cross, and variation.
        self.evol = Evolution_Prompt(api_endpoint, api_key, llm_model, debug_mode, problem_type , **kwargs)
        self.m = m                                 # Number of parent algorithms for prompt's cross operation
        self.debug = debug_mode                    # Whether debug mode is enabled

        # If debug mode is not enabled, warnings are suppressed
        if not self.debug:
            warnings.filterwarnings("ignore")

        self.select = select                       # Parent selection method
        self.n_p = n_p                             # Number of parallel processes/jobs
        self.timeout = timeout                     # Timeout definition

    # Writes the prompt text to `./prompt.txt`.
    ...
    # Provided that this new individual's prompt is not duplicated with other individuals in the population.
    # If the prompt is not duplicated, add it and return True; otherwise, return False.
    ...
    def extract_first_quoted_string(self, text):
        ...
        if text.startswith(prefix):
            return text[len(prefix):].strip()  # Remove prefix and leading/trailing whitespace
        return text  # If no match, return the original string


    # Used to generate offspring individuals based on the specified evolution operator
    def _get_alg(self, pop, operator):
        ...
        # Generate new prompt through crossover
            ...
            offspring["objective"] = 1e9
            offspring["number"] = []
        # Generate new prompt through variation
        ...
        # Return both selected parent algorithms and generated offspring
    ...
    def get_offspring(self, pop, operator):
        try:
            #print("Begin: 2")
            # Call the _get_alg method to generate offspring individuals from pop based on the operator (e.g., initial, cross, variation),
        ...
        # Return parent individuals p and generated offspring individuals
    ...
    def get_algorithm(self, pop, operator):
        ...
        try:
            # Generate self.pop_size offspring individuals. Results are stored in the results list,
            # each element being a (p, off) tuple, where p is the parent individual(s) and off is the generated offspring individual.
    ...
    def population_generation(self, initial_type):
        # Set to 2, indicating 2 rounds of individual generation.
        n_create = 1
...
class Evolution:
    ...
    def get_prompt_initial(self):
        #####################################################
        ### Generate the prompt for initial strategies ###
        #####################################################
        ...
        # 3. Tell LLM how many inputs and outputs this function has, and what they are (corresponding to processed self.prompt_func_inputs and self.prompt_func_outputs)
        # 4. Describe some properties of the input and output data.
        # 5. Describe some other supplementary properties.
        # 6. Finally, emphasize no other explanations, just strictly follow the requirements for output.
        prompt_content = self.prompt_task+"\n"\
...
+self.prompt_other_inf+"\n"+"Do not give additional explanations."
    ...
    def get_prompt_cross(self, indivs, prompt):
        #####################################################
        ### Generate a prompt for new heuristics that are as different as possible from parent heuristics ###
        #####################################################
...
+self.prompt_other_inf+"\n"+"Do not give additional explanations."
    ...
    def _get_alg(self, prompt_content):
        # Obtain the response for the given prompt content via the LLM interface.
        #print("QwQ~!")
        response = self.interface_llm.get_response(prompt_content)
        ...
        # Attempt to extract the algorithm description enclosed in curly braces {} from the response using the regex `re.findall(r"\{(.*)\}", response, re.DOTALL)`.
        # The re.DOTALL option allows the regex to match newline characters.
        algorithm = re.findall(r"\{(.*)\}", response, re.DOTALL)
        # If no algorithm description within curly braces is found, use alternative patterns.
        if len(algorithm) == 0:
            # If the response contains the word 'python', extract the part from the beginning up to the 'python' keyword as the algorithm description.
            if 'python' in response:
                algorithm = re.findall(r'^.*?(?=python)', response,re.DOTALL)
            # If it contains 'import', extract the part from the beginning up to 'import' as the algorithm description.
            elif 'import' in response:
                algorithm = re.findall(r'^.*?(?=import)', response,re.DOTALL)
            # Otherwise, extract the part from the beginning up to 'def' as the algorithm description.
            else:
                algorithm = re.findall(r'^.*?(?=def)', response,re.DOTALL)

        # Attempt to extract the code part using the regex `re.findall(r"import.*return", response, re.DOTALL)`,
        # where the code part starts with 'import' and ends with 'return'.
        code = re.findall(r"import.*return", response, re.DOTALL)
        # If no matching code block is found, try a code block starting with 'def' and ending with 'return'.
        if len(code) == 0:
        ...
        while (len(algorithm) == 0 or len(code) == 0):
            ...
            # Call the `get_response` method again to get a new response, and repeat attempts to extract the algorithm description and code.
            response = self.interface_llm.get_response(prompt_content)
            algorithm = re.findall(r"\{(.*)\}", response, re.DOTALL)
            if len(algorithm) == 0:
                if 'python' in response:
            ...
            # If the number of retries exceeds 3, exit the loop.
            if n_retry > 3:
                break
            n_retry += 1
        ...
        algorithm = algorithm[0] if algorithm else "" # Ensure algorithm is not empty if regex failed multiple times
        code = code[0] if code else "" # Ensure code is not empty if regex failed multiple times

        # The extracted code goes up to 'return'; we complete the rest.
        # Connect the extracted code with the output variables (stored in self.prompt_func_outputs) to form a complete code string.
    ...
    def initial(self):
        #####################################################
        ### Obtain algorithm description and code for creating the initial population ###
        #####################################################
        ...
        # In debug mode, output the returned text split into code and algorithm description
        if self.debug_mode:
...
def add_import_package_statement(program: str, package_name: str, as_name=None, *, check_imported=True) -> str:
    ...
    """
    # Parse the input Python code string into an Abstract Syntax Tree (AST) using ast.parse().
    ...
    if check_imported:
        ...
        for node in tree.body:
            # The first half checks if the node is an import statement.
            # The second half checks if the package name in the import statement is the same as package_name.
    ...
    # Create a new import node. Use ast.Import to create a new import statement node,
    # with the package name as package_name and alias as as_name (if any).
...
############################################################################################################
### Used to add the @numba.jit(nopython=True) decorator for NumPy to given Python code. ###
...
def _add_numba_decorator(
    ...
    # Traverse each node in the AST to check if 'import numba' statement already exists.
    ...
    for node in ast.walk(tree):
        ...
        if isinstance(node, ast.FunctionDef) and node.name == function_name:
            ...
            # ast.Call creates a call node, ast.Attribute represents attribute access (i.e., numba.jit),
            # and ast.keyword creates keyword nodes with named arguments.
            decorator = ast.Call(
                func=ast.Attribute(
                ...
                keywords=[ast.keyword(arg='nopython', value=ast.Constant(value=True))] # Change ast.NameConstant to ast.Constant for Python 3.8+
                # keywords have argument name
            )
            # Append it to the decorator_list attribute of the target function.
...
############################################################################################################
## Add a fixed random seed at the beginning of the function
############################################################################################################
...
def add_np_random_seed_below_numpy_import(program: str, seed: int = 2024) -> str:
    # This line calls the add_import_package_statement function to ensure that the program includes `import numpy as np`.
    ...
    for node in tree.body:
        ...
        if isinstance(node, ast.Import) and any(alias.name == 'numpy' and alias.asname == 'np' for alias in node.names):
            ...
            # This is achieved by creating a new AST node, representing a call to the `np.random.seed` function.
            ...
            seed_node = ast.Expr(
                value=ast.Call(
                    ...
                    args=[ast.Constant(value=seed)], # Change ast.Num to ast.Constant for Python 3.8+
                    keywords=[]
                )
            )
    ...
    # Use ast.unparse method to convert the modified syntax tree back into a Python code string, and return it.
    modified_code = ast.unparse(tree)
    return modified_code

# Used to add the `np.random.seed(seed)` statement within a specified Python function,
...
############################################################################################################
### Replaces division operators (/) in Python code with a custom protected division function _protected_div, ###
### and optionally uses the Numba library for accelerated computation. ###
############################################################################################################
# First, define the _CustomDivisionTransformer class, which inherits from ast.NodeTransformer.
# Its purpose is to traverse the Abstract Syntax Tree (AST), find all division operations (/),
# and replace them with calls to a custom division function.
class _CustomDivisionTransformer(ast.NodeTransformer):
    # It accepts a parameter custom_divide_func_name, which represents the name of the custom function
    # to be used to replace the division operator. Here, the name is _protected_div.
...
# Its purpose is to replace all division operators in the input code string with a custom protected division function
# named _protected_div, which avoids division by zero.
...
def _protected_div(x, y, delta={delta}):
    ...
    # Create an _CustomDivisionTransformer instance and traverse the AST, finding and replacing division operations.
    transformer = _CustomDivisionTransformer('_protected_div')
    modified_tree = transformer.visit(tree)

    # Convert the modified AST tree back into a code string;
    # here, the replaced code and the protected division function definition are returned together.
    modified_code = ast.unparse(modified_tree)
    modified_code = '\n'.join([modified_code, '', '', protected_div_str])

    # If numba_accelerate is true, add the @numba.jit() decorator to the _protected_div function to accelerate computation.
    if numba_accelerate:
...
#######################################
#######################################
### Communication and algorithm handling are all set, let's begin! ###
#######################################
#######################################
class InterfaceEC:
    def __init__(self, pop_size, m, api_endpoint, api_key, llm_model, debug_mode, interface_prob, select, n_p, timeout, use_numba, **kwargs):
        ...
        # Set information needed by LLM
        self.pop_size = pop_size                    # Define population size
        self.interface_eval = interface_prob        # PROBLEMCONST() type, which can call the evaluate function to assess algorithm code.
        prompts = interface_prob.prompts            # Prompts for problem description, input/output information, which can be used to generate subsequent prompts.
        self.evol = Evolution(api_endpoint, api_key, llm_model, debug_mode, prompts, **kwargs)  # Evolution type, includes initial, cross, and variation operators, which can be used for algorithm evolution.
        self.m = m                                  # Number of parent algorithms for 'cross' and 'variation' operations.
        self.debug = debug_mode                     # Whether debug mode is enabled
    ...
    # Writes the text code to file `./ael_alg.py`.
    ...
    # If there are no duplicate objective values, add it and return True; otherwise, return False.
    def add2pop(self, population, offspring):
        for off in offspring:
            for ind in population:
                if ind['objective'] == off['objective']:
                    if (self.debug):
                        print("duplicated result, retrying ... ")
                    return False # This logic seems to return False if ANY offspring duplicates ANY individual.
                                 # It might be intended to check if *this specific offspring* duplicates *any* existing, and then return.
                                 # Current implementation prevents adding any new offspring if *any* objective matches an *existing* one, which is strict.
                                 # Let's assume it wants to prevent adding the same objective value into the population.
                                 # The outer loop is over `offspring` (a list of generated algorithms), inner loop is `population` (existing algorithms).
                                 # If *any* `off` in `offspring` has an objective matching *any* `ind` in `population`, it returns False and stops adding.
                                 # This is problematic. It should add all valid offspring.
                                 # Let's fix this for `add2pop` and `add2pop_prompt` in EOH to iterate correctly and add non-duplicates.
        for off in offspring: # Re-iterating to add each offspring individually after checking against population.
            is_duplicate = False
            for ind in population:
                if ind['objective'] == off['objective']:
                    if self.debug:
                        print(f"duplicated objective {off['objective']}, skipping adding this offspring.")
                    is_duplicate = True
                    break
            if not is_duplicate:
                population.append(off)
        return True # Indicate that the process of attempting to add completed

    # Used to check if the given code snippet already exists in any individual within the population.
    # By checking for duplicate code snippets, redundant individuals can be avoided in the population.
    ...
    def _get_alg(self, pop, operator, prompt):
        ...
        # Generate new algorithms that improve current ones
        elif operator == "variation":
            parents = self.select.parent_selection(pop, 1)
    ...
    def get_offspring(self, pop, operator, prompt):
        ...
        try:
            ...
            if self.use_numba:
                # Use the regex `r"def\s+(\w+)\s*\(.*\):"` to match function definitions.
                pattern = r"def\s+(\w+)\s*\(.*\):"
                # Extract function name from offspring['code'].
                match = re.search(pattern, offspring['code'])
                function_name = match.group(1) if match else None # Handle case where no function name is found
                if function_name:
                    code = add_numba_decorator(program=offspring['code'], function_name=function_name)
                else: # Fallback if function name not found
                    code = offspring['code']
            else:
                code = offspring['code']
            ...
            while self.check_duplicate(pop, offspring['code']):
                ...
                p, offspring = self._get_alg(pop, operator, prompt) # Pass prompt again
                if self.use_numba:
                    pattern = r"def\s+(\w+)\s*\(.*\):"
                    match = re.search(pattern, offspring['code'])
                ...
                # Try at most once
                if n_retry > 1: # Original code was n_retry > 1, so it only retries once.
                    break
    ...
    def population_generation(self):
        ...
        # Loop to generate individuals
        for _ in range(n_create):
            _,pop = self.get_algorithm([], 'initial', []) # Pass empty list for prompt as well for initial
            for p in pop:
                population.append(p)
...
class EOH:
    ...
    def __init__(self, paras, problem, select, manage, **kwargs):
        ...
        self.operators = paras.ec_operators   # Define the number of operators; default are e1, e2, m1, m2.
        ...
        if paras.ec_m > self.pop_size or paras.ec_m == 1:    # Number of parents required for 'e1' and 'e2' operations; should be at least two but not exceed population size.
            print("m should not be larger than pop size or smaller than 2, adjust it to m=2")
            paras.ec_m = 2
        self.m = paras.ec_m                                  # Set the number of parents required for 'e1' and 'e2' operations.

        self.debug_mode = paras.exp_debug_mode               # Whether debug mode is enabled
        self.ndelay = 1  # default

        self.output_path = paras.exp_output_path             # Population results save path
    ...
    # Add newly generated offspring to the population; if debug mode is enabled, compare each with existing individuals in the population for redundancy.
    def add2pop(self, population, offsprings_list):
        for off in offsprings_list:
            if off['objective'] is None: # Skip invalid offspring
                continue
            is_duplicate = False
            for ind in population:
                if ind['objective'] == off['objective']:
    ...
    def add2pop_prompt(self, population, offsprings_list):
        for off in offsprings_list:
            is_duplicate = False
            for ind in population:
                if ind['prompt'] == off['prompt']:
                    if self.debug_mode:
                        print(f"duplicated prompt, skipping adding this prompt offspring.")
                    is_duplicate = True
                    break
            if not is_duplicate:
                population.append(off)
    ...
    def run(self):
        ...
        # Set up evolution mode, including initialization, evolution, and management
        ...
        for pop_gen in range(n_start, self.n_pop):
            ...
            for i in range(len(cross_operators)):
                ...
                parents_alg, offsprings_alg = interface_ec.get_algorithm(population, "cross", current_prompt)
                # Add newly generated offspring to the population; if debug mode is enabled,
                # compare each with existing individuals in the population for redundancy.
                self.add2pop(population, offsprings_alg)
                for off in offsprings_alg:
                    print(" Obj: ", off['objective'], end="|")
                    ...
                    if len(cross_operators[i]["number"]) < max_k:
                        ...
                        # If the heap is full and the current element is smaller than the heap's smallest element, replace the heap's smallest element.
                        if off['objective'] < -cross_operators[i]["number"][0]:
                            heapq.heapreplace(cross_operators[i]["number"], -off['objective'])  # Replace the smallest element
                ...
                #     with open(self.output_path + "/results/history/pop_" + str(pop_gen + 1) + "_" + str(
                #             na) + "_" + op + ".json", "w") as file:
                #         json.dump(data, file, indent=5)

                # After adding new offspring, if the population size exceeds, manage the population to keep the size at most pop_size.
                size_act = min(len(population), self.pop_size)
                population = self.manage.population_management(population, size_act)
                print(f"Cross {i + 1}, objective: {cross_operators[i]['objective']}", end = "|")
                print()

            # Next, consider the mutation operation.
            for i in range(len(variation_operators)):
                ...
                parents_alg, offsprings_alg = interface_ec.get_algorithm(population, "variation", current_prompt)
                # Add newly generated offspring to the population; if debug mode is enabled,
                # compare each with existing individuals in the population for redundancy.
                self.add2pop(population, offsprings_alg)
                for off in offsprings_alg:
                    ...
                    if len(variation_operators[i]["number"]) < max_k:
                        ...
                        if off['objective'] < -variation_operators[i]["number"][0]:
                            heapq.heapreplace(variation_operators[i]["number"], -off['objective'])  # Replace the smallest element
                    if variation_operators[i]["number"]: # Ensure number is not empty
            ...
            '''
            # This section was commented out in the original code, keeping it commented out.
            # For each operation, its operator_weights is the probability of running this operation
            for i in range(n_op):
                op = self.operators[i]
                print(f" OP: {op}, [{i + 1} / {n_op}] ", end="|")
                op_w = self.operator_weights[i]
                # If the random number is less than the weight (weight ranges from 0 to 1), then execute.
                if (np.random.rand() < op_w):
                    parents, offsprings = interface_ec.get_algorithm(population, op, []) # Pass empty list for prompt if not used
                # Add newly generated offspring to the population; if debug mode is enabled,
                # compare each with existing individuals in the population for redundancy.
